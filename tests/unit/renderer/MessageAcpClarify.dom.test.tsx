  ...
  return {
    ...
    content: {
      ...
      tool_call: {
        ...
        title: 'Soll ich im Linienmotiv die Linie von Grün auf Blau ändern?',
        kind: 'execute',
        raw_input: {
          question: 'Soll ich im Linienmotiv die Linie von Grün auf Blau ändern?',
...
describe('native Hermes clarify rendering', () => {
  ...
  it('keeps the artifact named by an already-visible card stable when a newer artifact arrives', async () => {
    const message = clarifyMessage();
    const events: unknown[] = [];
    const unsubscribe = addEventListener('commandEve.composer.followup.confirmed', (event) => events.push(event));
    ...
    expect(events[0]).toMatchObject({ artifact_id: 'img-latest' });
    unsubscribe();
  });
  ...
  it.each(['image', 'video', 'word', 'excel'] as const)(
    'resolves the latest exact %s artifact before offering the executable first choice',
    async (mode) => {
      artifactFixtures.current = [
        {
          id: `${mode}-latest`,
      ...
      const events: unknown[] = [];
      const unsubscribe = addEventListener('commandEve.composer.followup.confirmed', (event) => events.push(event));
      render(<MessageAcpPermission message={clarifyMessage('artifact_followup', mode)} isCommandEve />);

      expect(screen.getByTestId('message-acp-clarify-artifact')).toHaveTextContent(`${mode} latest`);
      fireEvent.click(screen.getByTestId('message-acp-clarify-option-0'));
      await waitFor(() => expect(events).toHaveLength(1));
      expect(events[0]).toMatchObject({ artifact_id: `${mode}-latest` });
      unsubscribe();
    }
  );
});
