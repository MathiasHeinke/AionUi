...
for f in AionUi-1.0.0-mac-x64.dmg AionUi-1.0.0-mac-arm64.dmg AionUi-1.0.0.deb AionUi-1.0.0-arm64.deb; do
  if [ ! -f "$OUTPUT_DIR/$f" ]; then
...
if find "$OUTPUT_DIR" -maxdepth 1 -type f \( -name '*.exe' -o -name '*.msi' -o -name 'latest.yml' -o -name '*-win-*' \) | grep -q .; then
  echo "FAIL: public release output contains a Windows pilot asset"
  ERRORS=$((ERRORS + 1))
else
  echo "PASS: public release output contains no Windows pilot assets"
fi
