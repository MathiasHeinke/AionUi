...
2. Top-left brand mark = orange Command glyph alone. Never inside a black rounded-square tile.
3. Conversation and project-chat rows use the Command glyph, not a chat bubble.
4. Row status badge only when meaningful: red = error/blocked, yellow = human action required, green = completed, blue = active/running or new result. No badge = idle.
5. Selected row is reduced and modern: faint glass wash + restrained accent hairline + stronger text/icon contrast. No heavy full-row slab, no card look.
6. Composer keeps the pointer-reactive orange border spotlight: neutral idle, localized warm hotspot near the pointer, keyboard focus ring, static fallbacks for touch/reduced
motion.
7. Seats = refined glass monograms. Active: neutral ring + online dot. Inactive: tiny identity-color rim indicator. Never fully saturated circles.
8. Paperclip stays on the same baseline as all other composer controls.
9. No purple CI. Orange brand, blue interaction accents, semantic red/yellow/green, neutral light/dark glass.
10. Background images remain local-first and must preserve readability.

4. Mockup-vs-Repo Adaptation Flags
...
Already shipped in default-color-scheme.css: --glass-chrome-bg/-solid/-border/-filter/-trail, --glass-panel-*, --glass-overlay-*, --glass-overlay-scrim, --glass-edge-highlight,
...
- Placement: top slot of the seat rail (SeatRail.tsx, above .seat-rail__toggle) for admins; for non-admin installs (rail hidden), the sidebar header layout-sider-header
...
/* command-eve-composer.css (new file, component-scoped) */
.eve-composer {
  position: relative;
...
/* Border-only warm hotspot: a masked gradient ring, never a surface glow. */
.eve-composer::before {
  content: '';
  position: absolute;
  inset: -1px;
...
/* Keyboard focus: honest ring, spotlight suppressed. */
...
/* Touch / no-hover: static warm hairline instead of tracking. */
...
2. Non-color encoding: every status has role='img' + aria-label + tooltip on hover and row focus; error/attention additionally announced via the polite live region (§7).
Hover-only affordances are never the sole path — the row ⋯ menu stays reachable via context-menu and keyboard.
3. Keyboard: conversation rows must be focusable and activatable (currently div onClick — ConversationRow.tsx:210; 1.8.0-c adds role='button', tabIndex=0, Enter/Space handling).
...
4. Target sizes: ≥ 28px effective targets everywhere (existing seat-rail and mobile-row conventions extend to new pills/buttons). Status dots are non-interactive and exempt.
5. Screen-reader names: brand glyph decorative (aria-hidden); privacy pill announces state in its accessible name, not color; seat buttons keep name + aria-current; composer
controls keep their existing labels through the slot contract.
6. Zoom/scaling: no clipping at 200% zoom and the app’s maximum font-size step, German locale included (chat-history__section max-height guard pattern; verified in the matrix).
7. Status announcements: exactly the §7 contract — no unlabeled color changes, no announcement spam.
8. Reduced transparency ≠ reduced motion: separate media queries, both tested (layout.css:57 pattern is the reference; the mobile chat-header combined query in chat-layout.css:39
...
- Long titles: single-line ellipsis + inline tooltip (existing chat-history__item-name + Tooltip); timestamps hidden on mobile (existing); German/max-font verified in the matrix.
...
│ glass)               │                                                                                                                                                       │
├──────────────────────┼───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
│ Tokens (new --eve-*) │ new packages/desktop/src/renderer/styles/themes/command-eve-visual.css                                                                                │
├──────────────────────┼───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
...
│ reference)           │                                                                                                                                                       │
├──────────────────────┼───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
│ i18n                 │ services/i18n/locales/*/conversation.json (+ modules per common/config/i18n-config.json)                                                              │
└──────────────────────┴───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
...
2. Attention-yellow contrast: #ca8a04 at 8px on light rows is ~2.9–3.2:1 against --bg-1; verify with the halo ring in place and darken one step if it fails the 3:1 UI-stroke
gate.
3. sessionStatus.ts remap blast radius: check every consumer of 'done' (tests, SessionStatusDot, any analytics/testids) before renaming to completed/newResult; run
gitnexus_impact on deriveSessionStatus at implementation time.
4. Spin retirement: confirm no other surface (e.g. batch mode, cron indicator, mobile) relied on the Spin overlay as its only busy signal; verify the pulse reads as “busy” in
user testing — if not, reinstate a spinner variant of the blue badge rather than the avatar-replacing Spin.
5. Row keyboard access: converting the row div to a focusable control interacts with the inner ⋯ Dropdown, Checkbox, and drag-and-drop (SortableConversationRow.tsx) — needs a
focus-management pass and e2e coverage.
6. :has(:focus-visible) support: verify the shipped Electron/Chromium version supports :has() in this position; fallback is a JS-toggled class from the existing focus handlers.
7. GroupedHistory directory limit: GroupedHistory/ is at 10 direct children; adding conversation-row.css violates AGENTS.md — resolve placement with the architecture skill (CSS
Module vs. subdirectory).
8. Seat identity rim at 14px collapsed size: a 2px arc on a 14px dot may be imperceptible; validate collapsed-rail identity legibility or accept tooltip-only identity when
collapsed.
9. --seat-rail-width invariants: the redesign must keep 72px/40px exactly or update the toast-centering consumers (layout.css:184).
10. Privacy pill component truth: confirm EgressRedactionTogglePill.tsx is the pill visible in the titlebar region shown in the mockups (vs. a separate Datenschutz header pill in
settings/onboarding surfaces) before styling.
11. Live-region wording: the polite badge-transition announcements (§7) need i18n review so German phrasing doesn’t read as an error toast.
12. Background scrim floor math: the +10% glass raise and --eve-bgimg-scrim-floor values are estimates; validate against the four reference images with real contrast sampling,
and check interaction with customCssProcessor.ts !important decorative themes.
13. Uncommitted state in this worktree: the design worktree currently carries uncommitted bug-fix-adjacent modifications (security dirs, default-color-scheme.css, layout.css);
reconcile ownership before slicing 1.8.0-b commits so visual diffs stay clean.
14. --info/--text-0/--border-special pruning: tokens.md marks them legacy-unused; decide whether 1.8.0-b prunes or defers.
