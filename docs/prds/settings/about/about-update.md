...
**用户故事**：作为用户，当有新版本可用时，我希望更新在后台完成下载，只在我主动打开更新详情时看到版本号、更新日志和重启操作。

**正常流程**（用户视角）：

1. Command EVE 在后台检查并下载更新，不自动打开弹窗
2. Footer 仅显示一个无文字的更新图标；tooltip 和状态点说明下载中、失败或已准备完成
3. 用户主动点击图标后，弹窗使用 medium 尺寸（内容高度 420px）
4. 顶部显示版本对比：`当前版本 → 新版本`（新版本号高亮）
5. 中间区域显示 release notes（Markdown 渲染，不启用 raw HTML）
6. 对 auto-update 路径显示“稍后”和“安装并重启”；manual fallback 保留原有下载操作：
   - 场景 A — Auto-update 可用：后台下载；完成后显示"稍后"与"安装并重启" → 进入 F-ABOUT-06
...
- [ ] Auto-update 的后台下载/重启操作与 manual fallback 按条件正确切换
- [ ] 无兼容安装包时有明确的警告提示
...
**用户故事**：作为用户，我希望系统静默下载更新，并在准备就绪后让我自行决定何时重启安装。

**前置条件**：electron-updater 确认有更新可用（auto-update 路径）

**正常流程**（用户视角）：

1. 启动后 3 秒检查更新；应用保持打开时每小时再次检查
2. electron-updater 自动在后台下载，不显示原生通知或阻塞弹窗
3. 下载状态由 Main Process 持久保存，renderer 重载或重新打开弹窗不会丢失进度/版本信息
4. 下载完成后 Footer 图标显示“已准备”状态，但不打断用户
5. 用户点击图标后查看版本、release notes，并选择“稍后”或“安装并重启”
6. 若用户跳过多个版本，最新 feed 版本替换已等待的旧包；一次重启直接安装最新版本
7. 只有用户点击“安装并重启”后应用退出并安装；普通退出不会自动安装

**平台差异**：

- macOS：在 `quitAndInstall` 前设置 quitting 状态，随后保持 graceful quit；禁止强制 `app.exit(0)`，否则会终止 Squirrel.Mac 的本地安装服务
...
- 下载中关闭弹窗：下载在后台继续运行；重新打开时从 durable Main-Process snapshot 恢复当前状态

**验收标准**：

- [ ] 更新状态不会自动打开弹窗或阻塞当前工作
- [ ] Footer control 只有更新图标，无常驻文字；tooltip 提供解释
- [ ] 下载过程显示实时进度（百分比、速度、已传输/总大小）
- [ ] 下载完成后显示"立即安装"按钮
- [ ] “稍后”不会在普通退出时隐式安装
- [ ] 多个跳过版本由最新版本取代，只需要一次重启
- [ ] 安装时应用正确退出并更新
- [ ] macOS 上安装流程能正确完成
- [ ] 下载/安装失败有明确错误提示
