...
export function fromBackendAgent(raw: unknown): TeamAgent {
  ...
  return {
    ...
    custom_agent_id: (r.custom_agent_id as string | undefined) ?? (r.assistant_id as string | undefined),
...
export function fromBackendTeam(raw: unknown): TTeam {
  const r = (raw ?? {}) as Record<string, unknown>;
  const rawAgents = Array.isArray(r.agents) ? r.agents : Array.isArray(r.assistants) ? r.assistants : [];
  const agents = (rawAgents as unknown[]).map(fromBackendAgent);
  return {
...
export function toBackendAgent(a: Omit<TeamAgent, 'slot_id' | 'conversation_id'>): Record<string, unknown> {
  const assistantId = a.custom_agent_id;
  return {
    name: a.agent_name,
    role: a.role === 'leader' ? 'lead' : a.role,
    ...(assistantId ? { assistant_id: assistantId } : { backend: a.agent_type }),
