...
export interface EveArtifactEnvelopeEntry {
   ...
   *
   * `image` covers TWO read paths, distinguishable by `editable`:
   * a SENT image (MAT-1769) is one the user attached to an EARLIER message,
   * re-listed from Main's durable record so a follow-up ("das Bild") resolves
   * the latest visible image without reattachment — newest first, never
   * editable, no handle. A MANAGED GENERATED image (1.820.3) is one the app
   * produced and bound to this conversation — editable, with an `evecap_`
   * handle minted under operation `image_edit`.
   */
...
export function buildEveArtifactContextEnvelope(input: EveArtifactContextEnvelopeInput): string {
  ...
  const header = [
    ...
    'Entries with kind=image are images from this conversation, NEWEST first — sent by you with an',
    'earlier message (editable=false) or generated for you by the app (editable=true, with a handle):',
