...
const catalog = {
  ...
  Goal: {
    description: 'EVE-native read-only goal summary.',
    props: {
      id: string(true),
      title: string(true),
      status: oneOf('planned', 'active', 'blocked', 'completed'),
  ...
  WorkerRun: {
    description: 'EVE-native worker-run receipt summary.',
    props: {
      id: string(true),
      worker: string(true),
      status: oneOf('queued', 'running', 'blocked', 'failed', 'completed'),
      startedAt: string(),
      summary: string(),
      receiptRef: string(),
    },
