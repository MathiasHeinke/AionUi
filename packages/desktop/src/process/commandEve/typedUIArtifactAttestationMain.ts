...
function readDurablePublishResult(message: TMessage, sourceMessageId: string, toolCallId: string): RecordValue {
  ...
  const payload = parseResultDisplay(matches[0].result_display);
  if (!payload || Object.keys(payload).some((key) => !RESULT_KEYS.has(key))) {
...
export async function attestDurableTypedUIArtifact(
  ...
  const result = readDurablePublishResult(message, request.artifact.source_message_id, toolCallId);
