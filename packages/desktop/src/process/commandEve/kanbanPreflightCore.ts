...
function parseMarketingCards(rows: unknown[]): CommandEveKanbanMarketingCard[] {
  return rows.map((row) => {
    ...
    return {
      ...
      output_approval_status: textField(item.output_approval_status) === 'approved' ? ('approved' as const) : null,
      output_approval_audit_event_id: nullableTextField(item.output_approval_audit_event_id),
      ...
      worker_dispatch_request_status:
        textField(item.worker_dispatch_request_status) === 'blocked' ? ('blocked' as const) : null,
      worker_dispatch_request_audit_event_id: nullableTextField(item.worker_dispatch_request_audit_event_id),
      worker_dispatch_requested_at:
        typeof item.worker_dispatch_requested_at === 'number' ? item.worker_dispatch_requested_at : null,
      worker_observed_run_status:
        textField(item.worker_observed_run_status) === 'completed' ? ('completed' as const) : null,
      ...
      worker_dispatcher_prepare_status:
        textField(item.worker_dispatcher_prepare_status) === 'ready' ? ('ready' as const) : null,
      worker_dispatcher_prepare_audit_event_id: nullableTextField(item.worker_dispatcher_prepare_audit_event_id),
...
try:
    ...
    for row in conn.execute(
        ...
        SELECT
          ...
          COALESCE(
            (
              SELECT json_extract(e.payload, '$.output_approval_status')
              FROM task_events e
              WHERE e.task_id = t.id
          ...
          COALESCE(
            ...
            ''
          ) AS output_approval_audit_event_id,
          COALESCE(
            (
              SELECT json_extract(e.payload, '$.output_source')
              FROM task_events e
              WHERE e.task_id = t.id
                AND e.kind = 'command_eve_marketing_output_approved'
                AND json_valid(e.payload)
          ...
          COALESCE(
            (
              SELECT json_extract(e.payload, '$.worker_dispatch_request_status')
              FROM task_events e
              WHERE e.task_id = t.id
                AND e.kind = 'command_eve_marketing_worker_dispatch_requested'
                AND json_valid(e.payload)
              ORDER BY e.created_at DESC, e.id DESC
              LIMIT 1
            ),
            ''
          ) AS worker_dispatch_request_status,
          COALESCE(
            (
              SELECT json_extract(e.payload, '$.audit_event_id')
              FROM task_events e
            ...
            ''
          ) AS worker_dispatch_request_audit_event_id,
          COALESCE(
            (
              SELECT e.created_at
              FROM task_events e
            ...
            0
          ) AS worker_dispatch_requested_at,
          COALESCE(
            (
              SELECT json_extract(e.payload, '$.worker_observed_run_status')
              FROM task_events e
              WHERE e.task_id = t.id
                AND e.kind = 'command_eve_marketing_worker_observed_run_completed'
                AND json_valid(e.payload)
              ORDER BY e.created_at DESC, e.id DESC
              LIMIT 1
            ),
            ''
          ) AS worker_observed_run_status,
          COALESCE(
            (
              SELECT json_extract(e.payload, '$.audit_event_id')
              FROM task_events e
            ...
            ''
          ) AS worker_observed_run_audit_event_id,
          COALESCE(
            (
              SELECT e.created_at
              FROM task_events e
          ...
          COALESCE(
            (
              SELECT json_extract(e.payload, '$.dispatcher_prepare_status')
              FROM task_events e
              WHERE e.task_id = t.id
                AND e.kind = 'command_eve_marketing_worker_dispatcher_prepared'
                AND json_valid(e.payload)
              ORDER BY e.created_at DESC, e.id DESC
              LIMIT 1
            ),
            ''
          ) AS worker_dispatcher_prepare_status,
          COALESCE(
            (
              SELECT json_extract(e.payload, '$.audit_event_id')
              FROM task_events e
            ...
            ''
          ) AS worker_dispatcher_prepare_audit_event_id,
          COALESCE(
            (
              SELECT e.created_at
              FROM task_events e
            ...
            0
          ) AS worker_dispatcher_prepared_at,
          substr(COALESCE(
            (
              SELECT json_extract(e.payload, '$.dispatcher_prepare_packet')
              FROM task_events e
              WHERE e.task_id = t.id
                AND e.kind = 'command_eve_marketing_worker_dispatcher_prepared'
                AND json_valid(e.payload)
