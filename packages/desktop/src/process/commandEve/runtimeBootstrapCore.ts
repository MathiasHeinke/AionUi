...
function writeHermesOllamaProviderOverride(paths: RuntimeBootstrapPaths): void {
  ...
  const initPy = [
    ...
    '#',
    '# We adopt a fresh, working session under the SAME id so the turn proceeds, and',
    '# we make the previously silent restore failures visible. The prior model',
    '# context of that chat is lost - that is stated in the log and surfaced by the',
    '# desktop; it is never swallowed.',
    'def _command_eve_adopt_acp_session(manager: Any, session_id: str, cwd: str = ".") -> Any:',
    '    import threading',
    '',
    '    from acp_adapter import session as acp_session_mod',
    '',
    '    translate = getattr(acp_session_mod, "_translate_acp_cwd", None)',
    '    resolved_cwd = translate(cwd) if callable(translate) else cwd',
    '    agent = manager._make_agent(session_id=session_id, cwd=resolved_cwd)',
    '    state = acp_session_mod.SessionState(',
    ...
    '    register = getattr(acp_session_mod, "_register_task_cwd", None)',
    '    if callable(register):',
    '        try:',
    '            register(session_id, resolved_cwd)',
    '        except Exception:',
    '            pass',
    '    try:',
    ...
    '    except Exception:',
    '        logging.getLogger(__name__).warning(',
    '            "Command EVE: adopted ACP session %s but could not persist it",',
    '            session_id,',
    '            exc_info=True,',
    '        )',
    '    logging.getLogger(__name__).warning(',
    '        "Command EVE: ACP session %s was unrecoverable; adopted a fresh session "',
    '        "under the same id so the turn can proceed (prior model context lost)",',
    '        session_id,',
    '    )',
    '    return state',
    '',
    '',
    'def _command_eve_recover_acp_session(manager: Any, session_id: str, cwd: str = ".") -> Any:',
    ...
    '        return existing',
    '    try:',
    '        return _command_eve_adopt_acp_session(manager, session_id, cwd)',
    '    except Exception:',
    '        logging.getLogger(__name__).error(',
    '            "Command EVE: ACP session %s could not be adopted", session_id, exc_info=True',
    '        )',
    '        return None',
    '',
    '',
    ...
    '        return',
    '',
    '    if getattr(HermesACPAgent, "_command_eve_acp_recovery_patch_installed", False):',
    '        return',
    '',
    ...
    '    original_prompt = getattr(HermesACPAgent, "_prompt_impl", None)',
    '    if not callable(original_load) or not callable(original_prompt):',
    '        return',
    '',
    ...
    '                _command_eve_recover_acp_session(manager, str(session_id), str(cwd or "."))',
    '        except Exception:',
    '            logging.getLogger(__name__).warning(',
    '                "Command EVE: ACP load-session self-heal failed", exc_info=True',
    '            )',
    ...
    '            manager = getattr(self, "session_manager", None)',
    '            if manager is not None and session_id:',
    '                _command_eve_recover_acp_session(manager, str(session_id))',
    '        except Exception:',
    '            logging.getLogger(__name__).warning(',
    '                "Command EVE: ACP prompt self-heal failed", exc_info=True',
    '            )',
