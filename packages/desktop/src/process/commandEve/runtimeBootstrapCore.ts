...
function writeHermesOllamaProviderOverride(paths: RuntimeBootstrapPaths): void {
  ...
  const initPy = [
    ...
    '    key = str(session_id or "").strip()',
    '    if key:',
    '        return bool(_COMMAND_EVE_MEMORY_QUARANTINE_BY_SESSION.get(key, False))',
    ...
    '    agent._command_eve_current_turn_has_attachment = (depth > 0 and outer_attachment) or _command_eve_prompt_has_attachment(prompt)',
    ...
    '    ) -> Any:',
    '        quarantined = bool(getattr(self, "_command_eve_current_turn_has_attachment", False)) or bool(',
    '            getattr(self, "_command_eve_current_turn_has_correction", False)',
    '        )',
    '        if quarantined:',
    '            review_memory = False',
    '            review_skills = False',
    ...
    '            accepted = (',
    '                bool(str(correction_text or "").strip()) and not bool(kwargs.get("hard_cancel", False))',
