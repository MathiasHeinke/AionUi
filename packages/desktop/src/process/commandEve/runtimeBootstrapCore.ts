...
function writeHermesOllamaProviderOverride(paths: RuntimeBootstrapPaths): void {
  ...
  const initPy = [
    ...
    '                        and existing_receipt.get("phase")',
    '                        in {"redirected", "boundary_sent"}',
    ...
    '                                or pending_receipt.get("phase")',
    '                                not in {"redirected", "boundary_sent"}',
    ...
    '',
    '                        try:',
    '                            if pending_receipt.get("phase") == "redirected":',
    '                                correction_update = acp.update_user_message_text(correction_text)',
    '                                correction_update.field_meta = {',
    '                                    "command_eve_control": "authoritative_correction"',
    '                                }',
    '                                await session_update(str(session_arg or ""), correction_update)',
    '                                with lock:',
    '                                    if getattr(state, receipt_attr, None) is not pending_receipt:',
    '                                        raise RuntimeError(',
    '                                            "Command EVE active correction receipt lost after boundary"',
    '                                        )',
    '                                    pending_receipt["phase"] = "boundary_sent"',
    '                            preview = correction_text[:80] + (',
    '                                "..." if len(correction_text) > 80 else ""',
    '                            )',
    '                            await session_update(',
    '                                str(session_arg or ""),',
    '                                acp.update_agent_message_text(',
    '                                    f"Correction accepted for the active turn: {preview}"',
    '                                ),',
    '                            )',
    ...
    '                            if getattr(state, receipt_attr, None) is not pending_receipt:',
    '                                raise RuntimeError(',
    '                                    "Command EVE active correction receipt lost after acknowledgement"',
    '                                )',
    '                            completed_corrections = getattr(',
    '                                state, "_command_eve_completed_active_corrections", None',
    '                            )',
