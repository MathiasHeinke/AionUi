...
function writeHermesOllamaProviderOverride(paths: RuntimeBootstrapPaths): void {
  ...
  const initPy = [
    ...
    '                        try:',
    '                            redirected = bool(redirect(correction_text)) if callable(redirect) else False',
    '                        except Exception:',
    '                            logging.warning(',
    ...
    '                            }',
    '                            await conn.session_update(str(session_arg or ""), correction_update)',
    '                            preview = correction_text[:80] + ("..." if len(correction_text) > 80 else "")',
