...
function writeHermesOllamaProviderOverride(paths: RuntimeBootstrapPaths): void {
  ...
  const initPy = [
    ...
    '        ) -> bool:',
    '            required = {"version", "request_id", "status"}',
    '            if not isinstance(value, dict) or set(value) != required:',
    '                raise RuntimeError("Command EVE correction boundary response shape rejected")',
    '            if value.get("version") != "command-eve-correction-boundary/v1":',
    ...
    '                                },',
    '                            )',
    '                            boundary_accepted = command_eve_validate_correction_boundary_response(',
    '                                response, request_id',
    '                            )',
