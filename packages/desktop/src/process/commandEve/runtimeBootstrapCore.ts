...
function writeHermesOllamaProviderOverride(paths: RuntimeBootstrapPaths): void {
  ...
  const initPy = [
    ...
    '        if agent is None or conn is None or not session_id:',
    ...
    '        agent._command_eve_current_turn_has_attachment = True',
    '        # Verified evidence needs a turn-scoped write gate, not a durable',
    '        # false-claim quarantine. Durable records are reserved for corrections',
    '        # and unverified claims; those records remain durable and fail closed.',
    ...
    '                # it produced no claims and can clear only its transient gate.',
    '                # Any older durable correction quarantine remains authoritative.',
    '                _command_eve_set_turn_memory_quarantine(session_id, False)',
