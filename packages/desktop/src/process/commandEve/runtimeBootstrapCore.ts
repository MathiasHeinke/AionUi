...
// Development-only keyless-web wheel fallback. Packaged macOS never copies raw
// native wheels into the app and never resolves this directory: DDGS and its
// exact closure are extracted into the deep-signed artifact Python site during
// the build, then verified by package/version/hash/origin at runtime.
...
export const EVE_STRATEGY_SKILL_IDS = [
  ...
  'presentation-studio',
  // office-studio is the deterministic DOCX/XLSX workflow selected by the
  // explicit Word/Excel composer modes. It uses only the signed document runtime
  // and always writes a new editable artifact instead of overwriting the source.
  'office-studio',
...
// Supabase's hosted Edge Runtime has a fixed 150s request-idle timeout and
// returns HTTP 504 when no response bytes arrive before it. Hermes retries an
// unchanged request with the unchanged oversized context, so its former three
// attempts could only repeat the same deadline and burn ~3 * 150s of user time.
// Keep one total API attempt; recovery must change the context before a new call.
const DEFAULT_COMMAND_EVE_API_MAX_RETRIES = 1;
...
// The absolute cap wins over Hermes' percentage floor. At the measured local
// summarization rate (52k tokens / 82s ~= 634 tokens/s), a 48k full summary
// projects to ~76s, so the unchanged 14s/29s cloud budget cannot be the normal
// path. Native proactive pruning is deterministic and LLM-free; it must reclaim
// old tool results toward 32k before the 48k full-compression fallback is needed.
const DEFAULT_COMMAND_EVE_COMPRESSION_THRESHOLD_TOKENS = 48_000;
const DEFAULT_COMMAND_EVE_PROACTIVE_PRUNE_TOKENS = 32_000;
const DEFAULT_COMMAND_EVE_PROACTIVE_PRUNE_MIN_RESULT_CHARS = 2_000;
const DEFAULT_COMMAND_EVE_PROACTIVE_PRUNE_MIN_RECLAIM_TOKENS = 4_096;
// Preserve roughly half of the active threshold. After the 48k fallback fires,
// the target working set is ~24k tokens rather than the wheel's fixed ~40-50k.
// The receipt still reports measured rough before/after counts rather than
// promising this target as fact.
...
export type RuntimeBootstrapPaths = {
  ...
  userDataPath: string;
  /**
   * Electron-owned real Command EVE data directory. On packaged macOS the
   * public CLI path (`~/.command-eve`) is intentionally a single symlink to
   * this directory; every managed descendant must still be a real entry.
   */
...
export type RuntimeBootstrapProvenance = {
  ...
  hermes?: {
    ...
    dependency_resolution: 'pypi_tls_on_first_boot' | 'packaged_signed_site';
    signed_site?: {
      path: string;
...
export type RuntimeBootstrapOptions = {
  userDataPath: string;
  /** Trusted real Electron data directory behind the optional macOS CLI alias. */
  canonicalUserDataPath?: string;
  ...
  /** Packaged macOS must use the exact signed interpreter and never fall through to a system Python. */
...
export const DEFAULT_COMMAND_EVE_CAPABILITY_PACK: CommandEveCapabilityPack = {
  ...
  skills: [
    ...
    {
      id: 'office-studio',
      name: 'Word & Excel Studio',
      tier: 'department',
      source: 'Command EVE managed document and spreadsheet production skill',
      default_state: 'active',
    },
    {
...
type CommandEveRuntimeProcessEnvDeps = CommandEveBrowserUseRunnerDeps & {
  ...
  requireBundledPython?: boolean;
  /** Already-verified signed Resources tree from the immediately preceding admission boundary. */
  verifiedPackagedArtifactSite?: Extract<CommandEveArtifactPythonSiteVerification, { ok: true }>;
};
...
export function resolveCommandEveRuntimeBootstrapPaths(
  ...
  seatId: string | null = getActiveSeatId(),
  platform: NodeJS.Platform = process.platform,
  canonicalUserDataPath: string = userDataPath
): RuntimeBootstrapPaths {
  const root = path.resolve(userDataPath || path.join(os.homedir(), '.command-eve'));
  const canonicalRoot = path.resolve(canonicalUserDataPath || root);
...
function verifiedPackagedArtifactSiteForResourcesPath(
  candidate: Extract<CommandEveArtifactPythonSiteVerification, { ok: true }> | undefined,
  resourcesPath: string | undefined
): Extract<CommandEveArtifactPythonSiteVerification, { ok: true }> | undefined {
  if (!candidate || !resourcesPath) return undefined;
  try {
    const expected = path.join(
      fs.realpathSync(path.resolve(resourcesPath)),
      'python',
      COMMAND_EVE_ARTIFACT_PYTHON_SITE_SUBDIR
    );
    return fs.realpathSync(candidate.directory) === expected ? candidate : undefined;
  } catch {
    return undefined;
  }
}
...
export function prepareCommandEveRuntimeProcessEnv(
  ...
  const verifiedPackagedArtifactSite = verifiedPackagedArtifactSiteForResourcesPath(
    browserUseRunnerDeps.verifiedPackagedArtifactSite,
    resourcesPath
  );
  const artifactSite = browserUseRunnerDeps.requireBundledPython
    ? (verifiedPackagedArtifactSite ?? resolveCommandEvePackagedArtifactPythonSite(resourcesPath))
    : resolveCommandEveArtifactPythonSite(env, resourcesPath);
  writeHermesCliShim(paths, artifactSite.ok ? artifactSite : undefined);
...
async function resolvePythonCommand(
  ...
  if (bundledCandidate && fs.existsSync(bundledCandidate)) {
    ...
    if (requireBundledCandidate) {
      return {
        ok: false,
        path: '',
        foundUnsupported: `The signed bundled Python interpreter is unavailable or invalid: ${bundledCandidate}.`,
      };
    }
    // Out-of-range/corrupt bundle: defensive fall-through for source/dev builds only.
  } else if (requireBundledCandidate) {
    return {
      ok: false,
      path: '',
      foundUnsupported: `The signed bundled Python interpreter is missing: ${bundledCandidate || '(unresolved)'}.`,
    };
  }
...
function resolveManagedUserDataRoot(paths: RuntimeBootstrapPaths): string | null {
  try {
    ...
    if (!canonicalStat?.isDirectory() || canonicalStat.isSymbolicLink()) return null;
    const canonicalReal = fs.realpathSync(paths.canonicalUserDataPath);
    const userDataStat = lstatManagedEntry(paths.userDataPath);
    if (!userDataStat) return null;

    if (userDataStat.isSymbolicLink()) {
      // Command EVE deliberately exposes exactly one CLI-safe alias on macOS.
      // Pin the link's immediate lexical target as well as its final realpath:
      // this rejects foreign, broken and chained aliases while still allowing
      // ~/.command-eve -> <Electron userData>/command-eve.
      if (
        paths.platform !== 'darwin' ||
        path.resolve(paths.userDataPath) === path.resolve(paths.canonicalUserDataPath)
      ) {
        return null;
      }
      const directTarget = path.resolve(path.dirname(paths.userDataPath), fs.readlinkSync(paths.userDataPath));
      if (directTarget !== path.resolve(paths.canonicalUserDataPath)) return null;
      const directTargetStat = fs.lstatSync(directTarget);
      if (!directTargetStat.isDirectory() || directTargetStat.isSymbolicLink()) return null;
      return fs.realpathSync(paths.userDataPath) === canonicalReal ? canonicalReal : null;
    }

    if (!userDataStat.isDirectory()) return null;
    if (path.resolve(paths.userDataPath) !== path.resolve(paths.canonicalUserDataPath)) return null;
    return fs.realpathSync(paths.userDataPath) === canonicalReal ? canonicalReal : null;
  } catch {
    return null;
  }
...
function managedRuntimeAncestryIsSafe(paths: RuntimeBootstrapPaths): boolean {
  try {
    ...
    const userDataReal = resolveManagedUserDataRoot(paths);
    if (!userDataReal) return false;
...
function assertManagedHermesRoot(paths: RuntimeBootstrapPaths): string {
  ...
  const userDataReal = resolveManagedUserDataRoot(paths);
  if (!userDataReal) throw new Error('unsafe user data root');
...
function synchronouslyProbeSelectedPython(candidate: string): string {
...
function exactRegularFileBytes(filePath: string, expected: string, expectedMode?: number): boolean {
  try {
...
function packagedRuntimeMutableArtifactsAreAdmissible(
  ...
  if (
    !pathFile ||
    !exactRegularFileBytes(pathFile, renderCommandEveArtifactPythonPathBinding(artifactSite.directory), 0o600)
  ) {
    return false;
  }
  ...
  for (const [name, moduleName, functionName] of COMMAND_EVE_PACKAGED_HERMES_LAUNCHERS) {
    ...
    if (!exactRegularFileBytes(path.join(binDirectory, name), expected, 0o700)) return false;
  }
  return (
    exactRegularFileBytes(paths.hermesShim, renderCommandEveHermesCliShim(paths, true), 0o700) &&
    exactRegularFileBytes(paths.hermesWrapper, renderCommandEveHermesWrapper(paths, true), 0o700)
  );
}
...
function runtimeReceiptProvesSelectedPython(
  ...
  try {
    const receipt = JSON.parse(fs.readFileSync(paths.receiptPath, 'utf8')) as Partial<RuntimeBootstrapReceipt>;
    const python = receipt.runtime_provenance?.python;
    const hermes = receipt.runtime_provenance?.hermes;
    const executableSha256 = sha256FileIfPresent(selectedPython);
    return Boolean(
      receipt.status === 'ready' &&
      path.resolve(String(receipt.runtime_root || '')) === path.resolve(paths.runtimeRoot) &&
      python?.source === 'bundled' &&
      path.resolve(String(python.executable || '')) === path.resolve(selectedPython) &&
      python.version === selectedVersion &&
      executableSha256 &&
      python.executable_sha256 === executableSha256 &&
      hermes?.dependency_resolution === 'packaged_signed_site' &&
      hermes.installed_wheel_verified === true &&
      hermes.signed_site?.path === artifactSite.directory &&
      hermes.signed_site.receipt_sha256 === artifactSite.receiptSha256 &&
...
export function commandEveRuntimeBootstrapStartupWaitReason(options: {
  ...
  if (!managedRuntimeAncestryIsSafe(paths)) return 'python_venv_recovery';
  const requireBundledPython = options.requireBundledPython === true;
  ...
  if (requireBundledPython) {
    const verification =
      verifiedPackagedArtifactSiteForResourcesPath(options.verifiedPackagedArtifactSite, options.resourcesPath) ??
  ...
  if (
    ...
    (!requireBundledPython && compact(env[COMMAND_EVE_BUNDLED_PYTHON_ENV]))
  ) {
  ...
  const selectedVersion = bundledVersion || selected.text;
  ...
  if (compatibility !== 'compatible') return 'python_abi_unproven';
  if (requireBundledPython) {
    if (
      !packagedArtifactSite ||
      !runtimeReceiptProvesSelectedPython(paths, bundledPython, selectedVersion, packagedArtifactSite)
    ) {
      return 'python_abi_unproven';
    }
    if (!packagedRuntimeMutableArtifactsAreAdmissible(paths, selectedVersion, packagedArtifactSite)) {
...
export function commandEveRuntimeVenvIsBackendAdmissible(options: {
  ...
  if (!managedRuntimeAncestryIsSafe(paths)) return false;
  const env = { ...process.env, ...options.env };
  const requireBundledPython = options.requireBundledPython === true;
  const bundledPython = resolveBundledPythonCandidate(requireBundledPython ? {} : env, options.resourcesPath, platform);
  const bundledVersion = requireBundledPython ? synchronouslyProbeSelectedPython(bundledPython) : '';
  if (requireBundledPython) {
    ...
    if (
      !bundledVersion ||
      !artifactSite.ok ||
      !runtimeReceiptProvesSelectedPython(paths, bundledPython, bundledVersion, artifactSite) ||
      !packagedRuntimeMutableArtifactsAreAdmissible(paths, bundledVersion, artifactSite)
    ) {
      return false;
    }
  }
...
export type CommandEveRuntimeBackendAdmissionResult =
  | {
      ok: true;
      artifactSite?: Extract<CommandEveArtifactPythonSiteVerification, { ok: true }>;
    }
  | {
      ok: false;
      reason: CommandEveRuntimeBootstrapStartupWaitReason;
      artifactSite?: Extract<CommandEveArtifactPythonSiteVerification, { ok: true }>;
    };
 ...
 *
 * The returned verified object may be carried only into the immediately
 * following environment bake. A later start boundary calls this function
 * again so mutation detection is never cached across time.
 */
export function inspectCommandEveRuntimeBackendAdmission(options: {
  userDataPath: string;
  canonicalUserDataPath?: string;
  resourcesPath?: string;
  env?: NodeJS.ProcessEnv;
  ...
  seatId?: string | null;
  requireBundledPython?: boolean;
}): CommandEveRuntimeBackendAdmissionResult {
  const artifactSite = options.requireBundledPython
    ? resolveCommandEvePackagedArtifactPythonSite(options.resourcesPath)
    : undefined;
  if (artifactSite && !artifactSite.ok) return { ok: false, reason: 'python_abi_unproven' };
  const sharedOptions = {
    ...options,
    ...(artifactSite?.ok ? { verifiedPackagedArtifactSite: artifactSite } : {}),
  };
  const reason = commandEveRuntimeBootstrapStartupWaitReason(sharedOptions);
  if (reason) return { ok: false, reason, ...(artifactSite?.ok ? { artifactSite } : {}) };
  if (!commandEveRuntimeVenvIsBackendAdmissible(sharedOptions)) {
    return { ok: false, reason: 'python_abi_unproven' };
  }
  return { ok: true, ...(artifactSite?.ok ? { artifactSite } : {}) };
}

async function repairCommandEvePackagedRuntimeMutableArtifacts(
  ...
  const binding = await bindCommandEveArtifactPythonSite({
    paths,
    artifactSite: artifactSite.directory,
    runner: options.runner ?? defaultRunner,
    env: { ...process.env, ...options.env },
  });
  if (binding.ok === false) throw new Error(`COMMAND_EVE_RUNTIME_PATH_BIND_FAILED: ${binding.reason}`);
  writePackagedHermesConsoleLaunchers(paths, artifactSite);
  writeHermesCommandWrapper(paths, artifactSite);
  writeHermesCliShim(paths, artifactSite);
}
...
export async function ensureCommandEveRuntimeBackendAdmission(
  ...
  let admission = inspectCommandEveRuntimeBackendAdmission(options);
  let receipt: RuntimeBootstrapReceipt | undefined;
  if (
    admission.ok === false &&
    admission.reason === 'runtime_artifact_invalid' &&
    admission.artifactSite &&
    options.requireBundledPython === true
  ) {
    // A normal seat transition changes the wrapper's baked fallback even though
    // the shared venv and signed receipt remain valid. Repair only the admitted
    // mutable surface here; a full bootstrap would rewrite target-seat config
    // with boot-seat settings after the old backend has already stopped.
    const platform = options.platform ?? process.platform;
    ...
    await withRuntimeBootstrapExclusive(platform, runtimeRoot, async () => {
      const current = inspectCommandEveRuntimeBackendAdmission(options);
      if (
        current.ok === false &&
        current.reason === 'runtime_artifact_invalid' &&
        current.artifactSite &&
        options.requireBundledPython === true
      ) {
        await repairCommandEvePackagedRuntimeMutableArtifacts(options, current.artifactSite);
      }
    });
  ...
  const paths = prepareCommandEveRuntimeProcessEnv(
    options.userDataPath,
    processEnv,
    options.platform ?? process.platform,
    options.resourcesPath,
    {
      requireBundledPython: options.requireBundledPython === true,
      ...(admission.artifactSite ? { verifiedPackagedArtifactSite: admission.artifactSite } : {}),
    }
  );
  return { admission, ...(receipt ? { receipt } : {}), paths };
}
...
async function bindCommandEveArtifactPythonSite(input: {
  ...
  try {
    ...
    if (fs.existsSync(pathFile)) {
      ...
      if (fs.readFileSync(pathFile, 'utf8') === binding) {
        fs.chmodSync(pathFile, 0o600);
        return { ok: true, pathFile };
      }
    }
    const temporary = `${pathFile}.${process.pid}.${Date.now()}.tmp`;
    fs.writeFileSync(temporary, binding, { encoding: 'utf8', mode: 0o600, flag: 'wx' });
...
export function renderCommandEvePackagedPythonLauncher(input: {
  ...
  if (
    [input.python, input.artifactSite, input.commandName, input.moduleName, input.functionName].some(
      (value) => !value || /[\r\n]/.test(value)
    )
  ) {
    throw new Error('packaged Hermes launcher input is invalid');
  }
  const pythonSource = [
    'import sys',
    `sys.path.insert(0, ${JSON.stringify(input.artifactSite)})`,
    `sys.argv[0] = ${JSON.stringify(input.commandName)}`,
    `from ${input.moduleName} import ${input.functionName}`,
    `raise SystemExit(${input.functionName}())`,
  ].join('; ');
  return [
    '#!/bin/sh',
    COMMAND_EVE_PACKAGED_HERMES_LAUNCHER_MARKER,
    `exec ${shellQuote(input.python)} -B -I -P -S -c ${shellQuote(pythonSource)} "$@"`,
    '',
  ].join('\n');
}

function writePackagedHermesConsoleLaunchers(
  paths: RuntimeBootstrapPaths,
  artifactSite: Extract<CommandEveArtifactPythonSiteVerification, { ok: true }>
): void {
  if (paths.platform === 'win32') throw new Error('packaged Hermes launchers are macOS-only');
  const hermesRootReal = assertManagedHermesRoot(paths);
  const venvReal = fs.realpathSync(paths.hermesVenv);
  if (path.dirname(venvReal) !== hermesRootReal || path.basename(venvReal) !== 'venv') {
    throw new Error('Hermes venv escaped its managed root');
  }
  const binDirectory = path.join(venvReal, 'bin');
  const binStat = fs.lstatSync(binDirectory);
  if (!binStat.isDirectory() || binStat.isSymbolicLink() || fs.realpathSync(binDirectory) !== binDirectory) {
  ...
  for (const [name, moduleName, functionName] of COMMAND_EVE_PACKAGED_HERMES_LAUNCHERS) {
    ...
    if (existing && (!existing.isFile() || existing.isSymbolicLink())) {
      throw new Error(`Hermes launcher is unsafe: ${name}`);
    }
    const source = renderCommandEvePackagedPythonLauncher({
      python,
      artifactSite: artifactSite.directory,
      commandName: name,
      moduleName,
    ...
    const temporary = `${target}.${process.pid}.${Date.now()}.tmp`;
    fs.writeFileSync(temporary, source, { encoding: 'utf8', mode: 0o700, flag: 'wx' });
    fs.renameSync(temporary, target);
    fs.chmodSync(target, 0o700);
  }
}

function writeHermesCommandWrapper(
  paths: RuntimeBootstrapPaths,
  artifactSite?: Extract<CommandEveArtifactPythonSiteVerification, { ok: true }>
): void {
  if (paths.platform === 'win32') return;
  ...
  const source = renderCommandEveHermesWrapper(
    paths,
    Boolean(artifactSite && packagedHermesConsoleLauncherIsExact(paths, artifactSite))
  );
...
function packagedHermesRuntimeProbeArgs(
  ...
  const expectedPackages = artifactSite.packages.map((entry) => [entry.name, entry.version]);
  const source = `
from importlib import import_module
from importlib.metadata import distribution
from pathlib import Path
import sys
root = Path(${JSON.stringify(artifactSite.directory)}).resolve(strict=True)
assert sys.flags.isolated == 1 and sys.flags.safe_path == 1 and sys.flags.no_site == 1 and sys.flags.ignore_environment == 1
sys.path.insert(0, str(root))
for package_name, expected_version in ${JSON.stringify(expectedPackages)}:
    installed = distribution(package_name)
    metadata_path = Path(str(installed._path)).resolve(strict=True)
    assert installed.version == expected_version, f"version mismatch for {package_name}: {installed.version}"
    assert metadata_path == root or root in metadata_path.parents, f"untrusted distribution origin for {package_name}: {metadata_path}"
def assert_module_origin(name):
    ...
    assert resolved == root or root in resolved.parents, f"untrusted module origin for {name}: {resolved}"
for module_name in ["hermes_cli.main", "acp_adapter.entry", "mcp", "openai", "ddgs", "primp", "lxml.etree"]:
    assert_module_origin(module_name)
assert distribution("hermes-agent").version == ${JSON.stringify(expectedVersion)}
print("COMMAND_EVE_PACKAGED_HERMES_RUNTIME_READY")
`;
 ...
 * install), or '' when none qualify. This resolver is never used by strict
 * packaged macOS bootstrap.
 */
...
function packagedHermesConsoleLauncherIsExact(
  paths: RuntimeBootstrapPaths,
  artifactSite: Extract<CommandEveArtifactPythonSiteVerification, { ok: true }>
): boolean {
  const [name, moduleName, functionName] = COMMAND_EVE_PACKAGED_HERMES_LAUNCHERS[0];
  return exactRegularFileBytes(
    hermesConsoleBinary(paths),
...
function writeHermesCliShim(
  ...
  if (paths.platform === 'win32') return;
  const shim = renderCommandEveHermesCliShim(
    paths,
    Boolean(artifactSite && packagedHermesConsoleLauncherIsExact(paths, artifactSite))
  );
...
function writeHermesOllamaProviderOverride(paths: RuntimeBootstrapPaths): void {
  ...
  const initPy = [
    ...
    '    "acp_internal_error_context",',
    '    "acp_epilogue_isolation",',
    ')',
    ...
    '_COMMAND_EVE_MEMORY_QUARANTINE_BY_SESSION: dict[str, bool] = {}',
    '_COMMAND_EVE_QUARANTINE_DB_BY_SESSION: dict[str, Any] = {}',
    '_COMMAND_EVE_CLAIM_QUARANTINE_VERSION = "command-eve-claim-quarantine/v1"',
    '_COMMAND_EVE_CLAIM_QUARANTINE_META_PREFIX = "command_eve.claim_quarantine.v1:"',
    '',
    '',
    ...
    '',
    '',
    'def _install_command_eve_media_runtime_cwd_patch() -> None:',
    '    """Resolve Hermes media paths against the active agent/session cwd.',
    '',
    '    Hermes 0.20 resolves bare relative media names against the Python process',
    '    cwd, unlike its terminal/file tools and prompt builder, which share',
    '    agent.runtime_cwd.resolve_agent_cwd(). Replace only that exact path anchor',
    '    before Hermes applies its existing host-cache/sandbox confinement checks.',
    '    """',
    '    try:',
    '        import textwrap',
    ...
    '    except Exception:',
    '        return',
    '    original = getattr(image_source, "resolve_image_source", None)',
    '    if not callable(original):',
    '        return',
    '    if getattr(original, "_command_eve_media_runtime_cwd_patch", False):',
    '        _command_eve_mark_patch("media_runtime_cwd")',
    '        return',
    '    try:',
    '        source = textwrap.dedent(inspect.getsource(original))',
    '    except Exception:',
    '        return',
    '    anchor = "    p = Path(os.path.expanduser(candidate))"',
    '    replacement = "\\n".join((',
    '        anchor,',
    ...
    '        return',
    '    patched_source = source.replace(anchor, replacement, 1)',
    '    namespace: dict[str, Any] = {}',
    '    try:',
    '        exec(',
    '            compile(',
    '                patched_source,',
    '                inspect.getsourcefile(original) or "<image_source>",',
    '                "exec",',
    '            ),',
    '            image_source.__dict__,',
    '            namespace,',
    '        )',
    '    except Exception:',
    '        return',
    '    patched = namespace.get("resolve_image_source")',
    '    if not callable(patched):',
    '        return',
    '    patched._command_eve_media_runtime_cwd_patch = True',
    '    image_source.resolve_image_source = patched',
    '    _command_eve_mark_patch("media_runtime_cwd")',
    '',
    '',
    ...
    '                "        ):\\n"',
    '                "            return None\\n"',
    '                "        companion = str(descriptor.get(\\"companion_path\\") or \\"\\").strip()\\n"',
    '                "        expected_companion = os.path.join(expected_root, \\"uv\\")\\n"',
    '                "        expected_companion_sha256 = str(descriptor.get(\\"companion_sha256\\") or \\"\\")\\n"',
    '                "        if (\\n"',
    '                "            companion != expected_companion\\n"',
    '                "            or not re.fullmatch(r\\"[a-f0-9]{64}\\", expected_companion_sha256)\\n"',
    '                "            or receipt.get(\\"companion_archive_entry\\")\\n"',
    '                "            != f\\"uv-{descriptor.get(\'target\')}/uv\\"\\n"',
    '                "            or receipt.get(\\"companion_filename\\") != os.path.basename(companion)\\n"',
    '                "            or receipt.get(\\"companion_sha256\\") != expected_companion_sha256\\n"',
    '                "        ):\\n"',
    '                "            return None\\n"',
    ...
    '                "            return None\\n"',
    '                "        if os.name != \\"nt\\":\\n"',
    '                "            if companion_info.st_mode & 0o077:\\n"',
    '                "                return None\\n"',
    '                "            if hasattr(os, \\"getuid\\") and companion_info.st_uid != os.getuid():\\n"',
    '                "                return None\\n"',
    '                "        if not os.access(companion, os.X_OK):\\n"',
    '                "            return None\\n"',
    '                "        real_companion = os.path.realpath(companion)\\n"',
    '                "        if (\\n"',
    '                "            real_companion != companion\\n"',
    '                "            or os.path.commonpath((real_root, real_companion)) != real_root\\n"',
    '                "        ):\\n"',
    '                "            return None\\n"',
    '                "        companion_flags = os.O_RDONLY\\n"',
    '                "        if hasattr(os, \\"O_NOFOLLOW\\"):\\n"',
    '                "            companion_flags |= os.O_NOFOLLOW\\n"',
    '                "        companion_fd = os.open(companion, companion_flags)\\n"',
    '                "        try:\\n"',
    '                "            opened_companion_info = os.fstat(companion_fd)\\n"',
    '                "            if not stat.S_ISREG(opened_companion_info.st_mode):\\n"',
    '                "                return None\\n"',
    '                "            companion_digest = hashlib.sha256()\\n"',
    '                "            while True:\\n"',
    '                "                chunk = os.read(companion_fd, 1024 * 1024)\\n"',
    '                "                if not chunk:\\n"',
    '                "                    break\\n"',
    '                "                companion_digest.update(chunk)\\n"',
    '                "        finally:\\n"',
    '                "            os.close(companion_fd)\\n"',
    '                "        if companion_digest.hexdigest() != expected_companion_sha256:\\n"',
    '                "            return None\\n"',
    '                "    except OSError:",',
    '            ),',
    '        )',
    ...
    '',
    '',
    'def _command_eve_emit_provider_turn_binding(',
    '    turn_id: str, api_request_id: str, api_call_count: int',
    ') -> None:',
    '    """Relay one content-free Hermes provider identity over the active ACP owner seam."""',
    '    try:',
    ...
    '    connections = globals().get("_COMMAND_EVE_DESKTOP_CONNECTIONS")',
    '    connections_lock = globals().get("_COMMAND_EVE_DESKTOP_CONNECTIONS_LOCK")',
    '    sent = globals().get("_COMMAND_EVE_PROVIDER_TURN_BINDINGS_SENT")',
    '    if (',
    '        not active_session_id',
    '        or not isinstance(connections, dict)',
    '        or connections_lock is None',
    '        or not isinstance(sent, set)',
    '    ):',
    '        return',
    '    binding_key = (active_session_id, api_request_id)',
    '    with connections_lock:',
    '        bridge = connections.get(active_session_id)',
    '        if bridge is None or binding_key in sent:',
    '            return',
    '        sent.add(binding_key)',
    ...
    '                "hermesTurnId": turn_id,',
    '                "requestId": api_request_id,',
    '                "callIndex": api_call_count,',
    ...
    '',
    '',
    'def _install_command_eve_provider_call_identity_patch() -> None:',
    '    """Stamp only the native Hermes 0.20 local /v1 chat-completions wire."""',
    '    try:',
    ...
    '    if getattr(manager, "_command_eve_provider_call_identity_patch", False):',
    '        _command_eve_mark_patch("provider_call_identity")',
    '        return',
    '',
    '    def command_eve_provider_call_identity_middleware(request: Any, **context: Any) -> dict[str, Any]:',
    '        if not isinstance(request, dict):',
    '            return {}',
    '        if context.get("provider") != "custom" or context.get("api_mode") != "chat_completions":',
    '            return {}',
    '        try:',
    '            base = urlparse(str(context.get("base_url") or ""))',
    '            port = base.port',
    '        except Exception:',
    '            return {}',
    ...
    '        if (',
    '            not re.fullmatch(r"[\\x21-\\x7e]{1,256}", turn_id)',
    ...
    '        stamped_headers = dict(headers or {})',
    '        stamped_headers["x-command-eve-turn-id"] = turn_id',
    '        stamped_headers["x-command-eve-call-index"] = str(api_call_count)',
    '        stamped["extra_headers"] = stamped_headers',
    '        try:',
    '            _command_eve_emit_provider_turn_binding(turn_id, api_request_id, api_call_count)',
    '        except Exception:',
    '            logging.getLogger(__name__).warning(',
    '                "Command EVE provider turn binding could not be relayed", exc_info=True',
    '            )',
    '        return {"request": stamped, "source": "command_eve_provider_call_identity"}',
    '',
    '    command_eve_provider_call_identity_middleware._command_eve_provider_call_identity_patch = True',
    '    PluginContext(',
    '        PluginManifest(name="command-eve-provider-call-identity"), manager',
    '    ).register_middleware(LLM_REQUEST_MIDDLEWARE, command_eve_provider_call_identity_middleware)',
    '    manager._command_eve_provider_call_identity_patch = True',
    '    _command_eve_mark_patch("provider_call_identity")',
    '',
    '',
    ...
    '',
    '',
    'def _command_eve_claim_quarantine_meta_key(session_id: Any) -> str:',
    '    key = str(session_id or "").strip()',
    '    return f"{_COMMAND_EVE_CLAIM_QUARANTINE_META_PREFIX}{key}" if key else ""',
    '',
    '',
    'def _command_eve_bind_claim_quarantine_db(agent: Any, session_id: Any, db: Any = None) -> Any:',
    '    key = str(session_id or "").strip()',
    '    if not key:',
    '        return None',
    '    candidate = db or getattr(agent, "_command_eve_quarantine_db", None) or getattr(agent, "_session_db", None)',
    '    if candidate is None or not callable(getattr(candidate, "get_meta", None)) or not callable(getattr(candidate, "set_meta", None)):',
    '        # An unreadable native store cannot prove that an older crash left no',
    '        # active quarantine. Fail closed for self-improvement in this session.',
    '        _COMMAND_EVE_MEMORY_QUARANTINE_BY_SESSION[key] = True',
    '        return None',
    '    agent._command_eve_quarantine_db = candidate',
    '    _COMMAND_EVE_QUARANTINE_DB_BY_SESSION[key] = candidate',
    '    return candidate',
    '',
    '',
    'def _command_eve_claim_quarantine_record_active(raw: Any) -> bool:',
    '    if raw is None:',
    '        return False',
    '    try:',
    '        record = json.loads(raw) if isinstance(raw, str) else None',
    '        required = {"version", "status", "reason", "turn_id", "evidence_digest", "updated_at_ms"}',
    '        if not isinstance(record, dict) or set(record) != required:',
    '            return True',
    '        if record.get("version") != _COMMAND_EVE_CLAIM_QUARANTINE_VERSION:',
    '            return True',
    '        status = record.get("status")',
    '        if status == "verified":',
    '            return not (',
    '                record.get("reason") == "attachment_grounding_verified"',
    '                and isinstance(record.get("turn_id"), str)',
    '                and bool(record.get("turn_id"))',
    '                and isinstance(record.get("evidence_digest"), str)',
    '                and bool(re.fullmatch(r"[a-f0-9]{64}", record.get("evidence_digest", "")))',
    '                and isinstance(record.get("updated_at_ms"), int)',
    '                and not isinstance(record.get("updated_at_ms"), bool)',
    '            )',
    '        if status != "active":',
    '            return True',
    '        if not isinstance(record.get("reason"), str) or not record.get("reason"):',
    '            return True',
    '        if not isinstance(record.get("turn_id"), str) or not isinstance(record.get("evidence_digest"), str):',
    '            return True',
    '        if isinstance(record.get("updated_at_ms"), bool) or not isinstance(record.get("updated_at_ms"), int):',
    '            return True',
    '        return True',
    '    except Exception:',
    '        return True',
    '',
    '',
    'def _command_eve_persist_claim_quarantine(',
    '    agent: Any, session_id: Any, *, status: str, reason: str, turn_id: str = "", evidence_digest: str = ""',
    ') -> bool:',
    '    key = str(session_id or "").strip()',
    '    db = _command_eve_bind_claim_quarantine_db(agent, key)',
    '    if db is None:',
    '        return False',
    '    record = {',
    '        "version": _COMMAND_EVE_CLAIM_QUARANTINE_VERSION,',
    '        "status": status,',
    '        "reason": str(reason or "unknown")[:64],',
    '        "turn_id": str(turn_id or "")[:256],',
    '        "evidence_digest": str(evidence_digest or "")[:128],',
    ...
    '    try:',
    '        db.set_meta(',
    '            _command_eve_claim_quarantine_meta_key(key),',
    ...
    '',
    '',
    'def _command_eve_persist_attachment_quarantine(agent: Any, session_id: Any, admission: dict[str, str]) -> bool:',
    '    key = str(session_id or "").strip()',
    '    db = _command_eve_bind_claim_quarantine_db(agent, key)',
    '    if db is None:',
    '        return False',
    '    try:',
    '        raw = db.get_meta(_command_eve_claim_quarantine_meta_key(key))',
    '        existing = json.loads(raw) if isinstance(raw, str) else None',
    '    except Exception:',
    '        _COMMAND_EVE_MEMORY_QUARANTINE_BY_SESSION[key] = True',
    '        return False',
    '    if _command_eve_claim_quarantine_record_active(raw):',
    '        existing_pending = (',
    '            isinstance(existing, dict)',
    '            and existing.get("version") == _COMMAND_EVE_CLAIM_QUARANTINE_VERSION',
    '            and existing.get("status") == "active"',
    '            and existing.get("reason") == "attachment_grounding_pending"',
    '            and isinstance(existing.get("turn_id"), str)',
    '            and bool(existing.get("turn_id"))',
    '            and isinstance(existing.get("evidence_digest"), str)',
    '            and bool(re.fullmatch(r"[a-f0-9]{64}", existing.get("evidence_digest", "")))',
    '            and isinstance(existing.get("updated_at_ms"), int)',
    '            and not isinstance(existing.get("updated_at_ms"), bool)',
    '        )',
    '        if not existing_pending:',
    '            # Never overwrite an older correction or malformed fail-closed',
    '            # record merely because a later attachment was verified.',
    '            required = {"version", "status", "reason", "turn_id", "evidence_digest", "updated_at_ms"}',
    '            existing_correction = (',
    '                isinstance(existing, dict)',
    '                and set(existing) == required',
    '                and existing.get("version") == _COMMAND_EVE_CLAIM_QUARANTINE_VERSION',
    '                and existing.get("status") == "active"',
    '                and existing.get("reason") == "user_correction"',
    '                and isinstance(existing.get("turn_id"), str)',
    '                and existing.get("evidence_digest") == ""',
    '                and isinstance(existing.get("updated_at_ms"), int)',
    '                and not isinstance(existing.get("updated_at_ms"), bool)',
    '            )',
    '            _COMMAND_EVE_MEMORY_QUARANTINE_BY_SESSION[key] = True',
    '            return bool(existing_correction)',
    '        # A crash-durable pending record is a retry fence, not a permanent',
    '        # session brick. Atomically advance it to the new exact turn/digest',
    '        # while keeping quarantine active throughout the retry.',
    '    return _command_eve_persist_claim_quarantine(',
    '        agent, key, status="active", reason="attachment_grounding_pending",',
    '        turn_id=admission.get("turn_id", ""), evidence_digest=admission.get("receipt_sha256", "")',
    '    )',
    '',
    '',
    'def _command_eve_verify_attachment_quarantine(agent: Any, session_id: Any, admission: dict[str, str]) -> bool:',
    '    key = str(session_id or "").strip()',
    '    db = _command_eve_bind_claim_quarantine_db(agent, key)',
    '    if db is None:',
    '        return False',
    ...
    '        and record.get("version") == _COMMAND_EVE_CLAIM_QUARANTINE_VERSION',
    '        and record.get("status") == "active"',
    '        and record.get("reason") == "attachment_grounding_pending"',
    '        and record.get("turn_id") == admission.get("turn_id")',
    '        and record.get("evidence_digest") == admission.get("receipt_sha256")',
    '    )',
    '    if not expected_pending:',
    '        # A valid older user correction remains authoritative and may share',
    '        # this grounded provider turn while its memory quarantine stays active.',
    '        # Missing, malformed, or unknown durable state cannot prove safety.',
    '        required = {"version", "status", "reason", "turn_id", "evidence_digest", "updated_at_ms"}',
    '        return bool(',
    '            isinstance(record, dict)',
    ...
    '            and record.get("version") == _COMMAND_EVE_CLAIM_QUARANTINE_VERSION',
    '            and record.get("status") == "active"',
    '            and record.get("reason") == "user_correction"',
    '            and isinstance(record.get("turn_id"), str)',
    '            and record.get("evidence_digest") == ""',
    '            and isinstance(record.get("updated_at_ms"), int)',
    '            and not isinstance(record.get("updated_at_ms"), bool)',
    '        )',
    '    verified = dict(record)',
    '    verified["status"] = "verified"',
    '    verified["reason"] = "attachment_grounding_verified"',
    '    verified["updated_at_ms"] = int(time.time() * 1000)',
    '    try:',
    '        db.set_meta(',
    '            _command_eve_claim_quarantine_meta_key(key),',
    ...
    '        _COMMAND_EVE_MEMORY_QUARANTINE_BY_SESSION[key] = True',
    '        return False',
    '    _COMMAND_EVE_MEMORY_QUARANTINE_BY_SESSION[key] = False',
    '    return True',
    '',
    '',
    'def _command_eve_set_turn_memory_quarantine(session_id: Any, quarantined: bool) -> None:',
    '    key = str(session_id or "").strip()',
    '    if not key:',
    '        return',
    '    _COMMAND_EVE_MEMORY_QUARANTINE_BY_SESSION[key] = bool(quarantined)',
    '',
    '',
    'def _command_eve_turn_memory_quarantined(session_id: Any) -> bool:',
    '    key = str(session_id or "").strip()',
    '    if key:',
    '        volatile = bool(_COMMAND_EVE_MEMORY_QUARANTINE_BY_SESSION.get(key, False))',
    '        db = _COMMAND_EVE_QUARANTINE_DB_BY_SESSION.get(key)',
    '        if db is None:',
    '            return volatile',
    '        try:',
    '            raw = db.get_meta(_command_eve_claim_quarantine_meta_key(key))',
    '        except Exception:',
    '            return True',
    '        return volatile or _command_eve_claim_quarantine_record_active(raw)',
    '    # Hermes 0.20 supplies session_id to pre_tool_call. If a future or',
    '    # degraded path omits it, fail closed only while some active session is',
    '    # quarantined; never guess that a mutation is safe.',
    '    return any(_COMMAND_EVE_MEMORY_QUARANTINE_BY_SESSION.values())',
    '',
    '',
    'def _command_eve_enter_turn_memory_quarantine(agent: Any, session_id: Any, prompt: Any) -> None:',
    '    depth = max(0, int(getattr(agent, "_command_eve_prompt_depth", 0) or 0))',
    '    correction = bool(getattr(agent, "_command_eve_current_turn_has_correction", False))',
    '    outer_attachment = bool(getattr(agent, "_command_eve_current_turn_has_attachment", False))',
    '    agent._command_eve_prompt_depth = depth + 1',
    '    agent._command_eve_session_id = str(session_id or "").strip()',
    '    agent._command_eve_current_turn_has_correction = correction',
    '    # Fail closed until attachment inspection succeeds. A marker failure',
    '    # may suppress one memory review; it must never let an ungrounded',
    '    # attachment claim become durable memory.',
    '    agent._command_eve_current_turn_has_attachment = True',
    '    agent._command_eve_current_turn_has_attachment = outer_attachment or _command_eve_prompt_has_attachment(prompt)',
    '    _command_eve_set_turn_memory_quarantine(',
    '        session_id, correction or agent._command_eve_current_turn_has_attachment',
    '    )',
    '',
    '',
    'def _command_eve_exit_turn_memory_quarantine(agent: Any, session_id: Any) -> None:',
    '    depth = max(0, int(getattr(agent, "_command_eve_prompt_depth", 1) or 1) - 1)',
    '    agent._command_eve_prompt_depth = depth',
    '    if depth > 0:',
    '        return',
    '    agent._command_eve_current_turn_has_attachment = False',
    '    agent._command_eve_current_turn_has_correction = False',
    '    _command_eve_set_turn_memory_quarantine(session_id, False)',
    '',
    '',
    ...
    '    ) -> Any:',
    '        session_id = str(getattr(self, "_command_eve_session_id", "") or getattr(self, "session_id", "") or "").strip()',
    '        _command_eve_bind_claim_quarantine_db(self, session_id)',
    '        quarantined = (',
    '            bool(getattr(self, "_command_eve_current_turn_has_attachment", False))',
    '            or bool(getattr(self, "_command_eve_current_turn_has_correction", False))',
    '            or _command_eve_turn_memory_quarantined(session_id)',
    '        )',
    '        if quarantined:',
    '            review_memory = False',
    '            review_skills = False',
    ...
    '',
    '    current_sync = getattr(AIAgent, "_sync_external_memory_for_turn", None)',
    '    if callable(current_sync) and not getattr(current_sync, "_command_eve_attachment_memory_gate", False):',
    '        def command_eve_sync_external_memory_for_turn(self: Any, *args: Any, **kwargs: Any) -> Any:',
    '            session_id = str(getattr(self, "_command_eve_session_id", "") or getattr(self, "session_id", "") or "").strip()',
    '            _command_eve_bind_claim_quarantine_db(self, session_id)',
    '            quarantined = (',
    '                bool(getattr(self, "_command_eve_current_turn_has_attachment", False))',
    ...
    '            if quarantined:',
    '                return None',
    '            return current_sync(self, *args, **kwargs)',
    '',
    '        command_eve_sync_external_memory_for_turn._command_eve_attachment_memory_gate = True',
    '        AIAgent._sync_external_memory_for_turn = command_eve_sync_external_memory_for_turn',
    '',
    '    # Hermes compression and session rotation bypass both background review',
    '    # and per-turn external sync through these direct on_session_end seams.',
    '    # A durable false-claim quarantine must block those writes too. Returning',
    '    # without flushing is deliberate: cleanup is safer to defer than to commit',
    '    # quarantined claims into any internal or external memory provider.',
    '    for boundary_method_name in ("commit_memory_session", "shutdown_memory_provider"):',
    '        current_boundary = getattr(AIAgent, boundary_method_name, None)',
    '        if not callable(current_boundary) or getattr(current_boundary, "_command_eve_attachment_memory_gate", False):',
    '            continue',
    '        def command_eve_memory_boundary(',
    '            self: Any, *args: Any, __current: Any = current_boundary, **kwargs: Any',
    '        ) -> Any:',
    '            session_id = str(getattr(self, "_command_eve_session_id", "") or getattr(self, "session_id", "") or "").strip()',
    '            _command_eve_bind_claim_quarantine_db(self, session_id)',
    '            quarantined = (',
    ...
    '',
    '        command_eve_memory_boundary._command_eve_attachment_memory_gate = True',
    '        setattr(AIAgent, boundary_method_name, command_eve_memory_boundary)',
    '',
    '    for correction_method_name in ("steer", "redirect", "interrupt"):',
    '        current_correction = getattr(AIAgent, correction_method_name, None)',
    '        if not callable(current_correction) or getattr(current_correction, "_command_eve_correction_gate", False):',
    '            continue',
    '        def command_eve_correction(',
    '            self: Any,',
    '            *args: Any,',
    '            __current: Any = current_correction,',
    '            __method: str = correction_method_name,',
    '            **kwargs: Any,',
    '        ) -> Any:',
    '            correction_text = args[0] if args else kwargs.get("message", kwargs.get("text"))',
    '            candidate = bool(str(correction_text or "").strip()) and not bool(kwargs.get("hard_cancel", False))',
    '            session_id = str(getattr(self, "_command_eve_session_id", "") or getattr(self, "session_id", "") or "").strip()',
    '            _command_eve_bind_claim_quarantine_db(self, session_id)',
    '            if candidate:',
    '                self._command_eve_quarantine_reason = "user_correction"',
    '                self._command_eve_quarantine_turn_id = str(getattr(self, "_command_eve_current_turn_id", "") or "")',
    '                self._command_eve_quarantine_evidence_digest = ""',
    '                if not _command_eve_persist_claim_quarantine(',
    '                    self, session_id, status="active", reason="user_correction",',
    '                    turn_id=self._command_eve_quarantine_turn_id, evidence_digest=""',
    '                ):',
    '                    raise RuntimeError("Command EVE claim quarantine persistence unavailable")',
    '            result = __current(self, *args, **kwargs)',
    '            accepted = (',
    '                candidate',
    '                if __method == "interrupt"',
    '                else bool(result)',
    '            )',
    '            if accepted:',
    '                self._command_eve_current_turn_has_correction = True',
    '                _command_eve_set_turn_memory_quarantine(',
    '                    getattr(self, "_command_eve_session_id", ""), True',
    '                )',
    '            return result',
    '        command_eve_correction._command_eve_correction_gate = True',
    '        setattr(AIAgent, correction_method_name, command_eve_correction)',
    '    _command_eve_mark_patch("attachment_memory_gate")',
    '',
    '',
    ...
    '',
    '',
    'def _command_eve_prompt_is_correction(prompt: Any, state: Any) -> bool:',
    '    text = _command_eve_prompt_text(prompt)',
    '    if not text:',
    '        return False',
    '    command = text.split(maxsplit=1)[0].lower()',
    '    if command in {"/correct", "/steer"}:',
    '        return True',
    '    if state is None:',
    '        return False',
    '    lock = getattr(state, "runtime_lock", None)',
    '    if lock is None:',
    '        return bool(getattr(state, "is_running", False) or getattr(state, "interrupted_prompt_text", ""))',
    '    with lock:',
    '        return bool(getattr(state, "is_running", False) or getattr(state, "interrupted_prompt_text", ""))',
    '',
    '',
    'def _install_command_eve_prompt_admission_patch() -> None:',
    '    try:',
    '        from acp_adapter.server import HermesACPAgent',
    '        from run_agent import AIAgent',
    '    except Exception:',
    '        return',
    '    if getattr(HermesACPAgent, "_command_eve_prompt_admission_patch_installed", False):',
    '        _command_eve_mark_patch("prompt_admission")',
    '        return',
    '    original_prompt = getattr(HermesACPAgent, "prompt", None)',
    '    original_drain = getattr(HermesACPAgent, "_drain_queued_prompts", None)',
    '    original_run = getattr(AIAgent, "run_conversation", None)',
    '    if not callable(original_prompt) or not callable(original_drain) or not callable(original_run):',
    '        raise RuntimeError("Command EVE prompt admission requires Hermes 0.20 prompt/drain/run_conversation seams")',
    ...
    '',
    '        class _CommandEveCorrectionDrainFenced(RuntimeError):',
    '            pass',
    '',
    '        class _CommandEveCorrectionFencedQueue(list[Any]):',
    '            def __init__(self, state: Any, values: Any) -> None:',
    '                super().__init__(values)',
    '                self._command_eve_state = state',
    '',
    '            def pop(self, index: int = -1) -> Any:',
    '                if getattr(',
    '                    self._command_eve_state,',
    '                    "_command_eve_active_correction_receipt",',
    '                    None,',
    '                ) is not None:',
    '                    candidate = super().__getitem__(index)',
    '                    if hasattr(candidate, "claimed"):',
    '                        candidate.claimed = False',
    '                    raise _CommandEveCorrectionDrainFenced()',
    '                return super().pop(index)',
    '',
    '        async def command_eve_drain_queued_prompts(self: Any, *args: Any, **kwargs: Any) -> None:',
    '            state = kwargs.get("state") if "state" in kwargs else (args[0] if args else None)',
    '            lock = getattr(state, "runtime_lock", None) if state is not None else None',
    '            if lock is not None:',
    '                with lock:',
    '                    queue = getattr(state, "queued_prompts", None)',
    '                    if not isinstance(queue, list):',
    '                        raise RuntimeError("Command EVE correction fence requires the native prompt list")',
    '                    if not isinstance(queue, _CommandEveCorrectionFencedQueue):',
    '                        state.queued_prompts = _CommandEveCorrectionFencedQueue(state, queue)',
    '                    if getattr(state, "_command_eve_active_correction_receipt", None) is not None:',
    '                        return',
    '            try:',
    '                await original_drain(self, *args, **kwargs)',
    '            except _CommandEveCorrectionDrainFenced:',
    '                return',
    '',
    '        command_eve_drain_queued_prompts._command_eve_correction_fence = True',
    '',
    '        def command_eve_validate_correction_boundary_response(',
    '            value: Any, request_id: str',
    '        ) -> str:',
    '            required = {"version", "request_id", "status", "rejection"}',
    '            if not isinstance(value, dict) or set(value) != required:',
    '                raise RuntimeError("Command EVE correction boundary response shape rejected")',
    '            if value.get("version") != "command-eve-correction-boundary/v2":',
    '                raise RuntimeError("Command EVE correction boundary response version mismatch")',
    '            if value.get("request_id") != request_id:',
    '                raise RuntimeError("Command EVE correction boundary response identity mismatch")',
    '            status = value.get("status")',
    '            rejection = value.get("rejection")',
    '            if status == "accepted" and rejection is None:',
    '                return "accepted"',
    '            if status == "rejected" and rejection in {',
    '                "retryable_same_generation",',
    '                "lifecycle_changed",',
    '                "session_mismatch",',
    '            }:',
    '                return rejection',
    '            raise RuntimeError("Command EVE correction boundary status rejected")',
    '',
    ...
    '                        completed_corrections = None',
    '                    else:',
    '                        with lock:',
    '                            is_running = bool(getattr(state, "is_running", False))',
    '                            has_interrupted = bool(getattr(state, "interrupted_prompt_text", ""))',
    '                            existing_receipt = getattr(',
    '                                state, "_command_eve_active_correction_receipt", None',
    '                            )',
    '                            completed_corrections = getattr(',
    '                                state, "_command_eve_completed_active_corrections", None',
    '                            )',
    '                    retrying_receipt = (',
    '                        isinstance(existing_receipt, dict)',
    '                        and existing_receipt.get("text") == correction_text',
    '                        and existing_receipt.get("phase") == "redirected"',
    '                        and not bool(existing_receipt.get("in_flight"))',
    '                    )',
    '                    completed_replay = (',
    '                        isinstance(completed_corrections, set)',
    '                        and correction_text in completed_corrections',
    '                    )',
    '                    if completed_replay:',
    '                        from acp.schema import PromptResponse',
    '',
    '                        return PromptResponse(stop_reason="end_turn")',
    '                    if existing_receipt is not None and not retrying_receipt:',
    '                        from acp.schema import PromptResponse',
    '',
    '                        return PromptResponse(stop_reason="refusal")',
    '                    if is_running or retrying_receipt:',
    '                        if not correction_text:',
    '                            from acp.schema import PromptResponse',
    '',
    '                            return PromptResponse(stop_reason="refusal")',
    '                        conn = getattr(self, "_conn", None)',
    '                        ext_method = getattr(conn, "ext_method", None)',
    '                        supports_redirect = (',
    '                            getattr(',
    '                                getattr(state, "agent", None),',
    ...
    '                        )',
    '                        if (',
    '                            not callable(ext_method)',
    '                            or lock is None',
    '                            or not supports_redirect',
    '                        ):',
    '                            # A correction is authoritative only when its ACP',
    '                            # boundary can be committed atomically with the',
    '                            # redirect receipt. Never mutate the running agent',
    '                            # first and discover a missing transport afterwards.',
    '                            from acp.schema import PromptResponse',
    '',
    '                            return PromptResponse(stop_reason="refusal")',
    '                        receipt_attr = "_command_eve_active_correction_receipt"',
    '                        with lock:',
    '                            queued_prompts = getattr(state, "queued_prompts", None)',
    '                            pending_receipt = getattr(state, receipt_attr, None)',
    '                            owns_redirect = pending_receipt is None',
    '                            if owns_redirect:',
    '                                if not isinstance(queued_prompts, list) or queued_prompts:',
    '                                    from acp.schema import PromptResponse',
    '',
    '                                    return PromptResponse(stop_reason="refusal")',
    '                                import uuid',
    ...
    '                                    "in_flight": True,',
    '                                }',
    '                                setattr(state, receipt_attr, pending_receipt)',
    '                            elif (',
    '                                not isinstance(pending_receipt, dict)',
    '                                or pending_receipt.get("text") != correction_text',
    '                                or pending_receipt.get("phase") != "redirected"',
    '                                or not isinstance(pending_receipt.get("request_id"), str)',
    '                                or bool(pending_receipt.get("in_flight"))',
    '                            ):',
    '                                from acp.schema import PromptResponse',
    '',
    '                                return PromptResponse(stop_reason="refusal")',
    '                            else:',
    '                                pending_receipt["in_flight"] = True',
    '                        import logging',
    '',
    '                        redirect = getattr(getattr(state, "agent", None), "redirect", None)',
    '                        if owns_redirect:',
    '                            try:',
    '                                redirected = (',
    '                                    bool(redirect(correction_text)) if callable(redirect) else False',
    '                                )',
    '                            except Exception:',
    '                                logging.warning(',
    '                                    "Command EVE active correction redirect failed for %s",',
    '                                    str(session_arg or ""),',
    '                                    exc_info=True,',
    '                                )',
    '                                redirected = False',
    '                            if not redirected:',
    '                                with lock:',
    '                                    if getattr(state, receipt_attr, None) is pending_receipt:',
    '                                        delattr(state, receipt_attr)',
    '                                # The model/tool boundary crossed before the correction',
    '                                # could be admitted. Reject instead of silently queueing',
    '                                # a control prompt whose authority can no longer be proven.',
    '                                from acp.schema import PromptResponse',
    '',
    '                                return PromptResponse(stop_reason="refusal")',
    '                            with lock:',
    '                                if getattr(state, receipt_attr, None) is not pending_receipt:',
    '                                    raise RuntimeError(',
    '                                        "Command EVE active correction receipt lost after redirect"',
    '                                    )',
    '                                pending_receipt["phase"] = "redirected"',
    '                        try:',
    '                            request_id = pending_receipt["request_id"]',
    '                            response = await ext_method(',
    '                                "command_eve/correction_boundary",',
    '                                {',
    '                                    "version": "command-eve-correction-boundary/v2",',
    '                                    "request_id": request_id,',
    '                                    "session_id": str(session_arg or ""),',
    '                                },',
    '                            )',
    '                            boundary_outcome = command_eve_validate_correction_boundary_response(',
    '                                response, request_id',
    '                            )',
    '                        except BaseException:',
    '                            with lock:',
    '                                if getattr(state, receipt_attr, None) is pending_receipt:',
    '                                    pending_receipt["in_flight"] = False',
    '                            raise',
    '                        if boundary_outcome != "accepted":',
    '                            dropped_queue_count = 0',
    '                            with lock:',
    '                                if getattr(state, receipt_attr, None) is pending_receipt:',
    '                                    if boundary_outcome == "retryable_same_generation":',
    '                                        pending_receipt["in_flight"] = False',
    '                                    else:',
    '                                        delattr(state, receipt_attr)',
    '                                        queued_prompts = getattr(state, "queued_prompts", None)',
    '                                        if isinstance(queued_prompts, list):',
    '                                            dropped_queue_count = len(queued_prompts)',
    '                                            queued_prompts.clear()',
    '                            if dropped_queue_count:',
    '                                session_update = getattr(getattr(self, "_conn", None), "session_update", None)',
    '                                if callable(session_update):',
    '                                    import acp',
    '',
    ...
    '                            if getattr(state, receipt_attr, None) is not pending_receipt:',
    '                                raise RuntimeError(',
    '                                    "Command EVE active correction receipt lost after boundary acknowledgement"',
    '                                )',
    '                            completed_corrections = getattr(',
    '                                state, "_command_eve_completed_active_corrections", None',
    '                            )',
    '                            if not isinstance(completed_corrections, set):',
    '                                completed_corrections = set()',
    '                                setattr(',
    '                                    state,',
    '                                    "_command_eve_completed_active_corrections",',
    '                                    completed_corrections,',
    '                                )',
    '                            completed_corrections.add(correction_text)',
    '                            delattr(state, receipt_attr)',
    '                        await self._drain_queued_prompts(state)',
    '                        from acp.schema import PromptResponse',
    '',
    '                        return PromptResponse(stop_reason="end_turn")',
    '                    if not is_running:',
    ...
    '                        with lock:',
    '                            if hasattr(',
    '                                state, "_command_eve_completed_active_corrections"',
    '                            ):',
    '                                delattr(',
    '                                    state, "_command_eve_completed_active_corrections"',
    '                                )',
    ...
    '                queued_depth = None',
    '                ordinary_text_prompt = (',
    '                    bool(prompt_text)',
    '                    and not prompt_text.startswith("/")',
    '                    and not bool(kwargs.get("_goal_continuation"))',
    '                    and not bool(kwargs.get("_heartbeat_continuation"))',
    '                )',
    '                if state is not None and lock is not None:',
    '                    with lock:',
    '                        is_running = bool(getattr(state, "is_running", False))',
    '                        pending_receipt = getattr(',
    '                            state, "_command_eve_active_correction_receipt", None',
    '                        )',
    '                        if ordinary_text_prompt and pending_receipt is not None:',
    '                            blocked_by_correction = True',
    '                        elif is_running and ordinary_text_prompt:',
    ...
    '                                    is True',
    '                                )',
    '                                if supports_redirect and callable(redirect):',
    '                                    try:',
    '                                        redirected = bool(redirect(prompt_text))',
    '                                    except Exception:',
    '                                        import logging',
    '',
    '                                        logging.debug(',
    '                                            "Command EVE active-turn redirect failed for %s",',
    '                                            str(session_arg or ""),',
    '                                            exc_info=True,',
    '                                        )',
    '                            if not redirected and not blocked_by_correction:',
    '                                state.queued_prompts.append(prompt_text)',
    ...
    '                    conn = getattr(self, "_conn", None)',
    '                    session_update = getattr(conn, "session_update", None)',
    '                    if callable(session_update):',
    '                        import acp',
    '',
    '                        await session_update(',
    ...
    '',
    '                    return PromptResponse(stop_reason="refusal")',
    '                if redirected or queued_depth is not None:',
    '                    conn = getattr(self, "_conn", None)',
    '                    session_update = getattr(conn, "session_update", None)',
    '                    if callable(session_update):',
    '                        import acp',
    ...
    '                            if redirected',
    '                            else f"Queued for the next turn. ({queued_depth} queued)"',
    '                        )',
    '                        await session_update(',
    '                            str(session_arg or ""), acp.update_agent_message_text(message)',
    '                        )',
    '                    from acp.schema import PromptResponse',
    '',
    '                    return PromptResponse(stop_reason="end_turn")',
    ...
    '        HermesACPAgent._drain_queued_prompts = command_eve_drain_queued_prompts',
    ...
    '        if not isinstance(value, dict) or set(value) != required:',
    '            raise ValueError("Command EVE prompt admission shape rejected")',
    '        normalized = {key: str(value.get(key) or "").strip() for key in required}',
    '        if normalized["version"] != "command-eve-prompt-admission/v1":',
    '            raise ValueError("Command EVE prompt admission version mismatch")',
    '        if any(not normalized[key] or len(normalized[key]) > 256 for key in ("request_id", "turn_id")):',
    '            raise ValueError("Command EVE prompt admission identifier rejected")',
    '        digest = normalized["receipt_sha256"]',
    '        if len(digest) != 64 or any(ch not in "0123456789abcdef" for ch in digest):',
    '            raise ValueError("Command EVE prompt admission digest rejected")',
    '        return normalized',
    '',
    '    async def command_eve_prompt(self: Any, *args: Any, **kwargs: Any) -> Any:',
    '        raw_admission = kwargs.pop("commandEvePromptAdmission", None)',
    '        prompt_arg = kwargs.get("prompt") if "prompt" in kwargs else (args[0] if args else None)',
    '        session_arg = kwargs.get("session_id")',
    '        if session_arg is None and len(args) > 1:',
    '            session_arg = args[1]',
    '        session_id = str(session_arg or "").strip()',
    '        manager = getattr(self, "session_manager", None)',
    '        state = manager.get_session(session_id) if manager is not None and session_id else None',
    '        if session_id:',
    '            cwd = str(getattr(state, "cwd", "") or "").strip() if state is not None else ""',
    '            if cwd:',
    '                _COMMAND_EVE_SESSION_CWD[session_id] = cwd',
    '            else:',
    '                _COMMAND_EVE_SESSION_CWD.pop(session_id, None)',
    '        agent = getattr(state, "agent", None)',
    '        correction = _command_eve_prompt_is_correction(prompt_arg, state)',
    '        if raw_admission is None and not correction:',
    '            return await original_prompt(self, *args, **kwargs)',
    '        conn = getattr(self, "_conn", None)',
    '        if agent is None or not session_id or (raw_admission is not None and conn is None):',
    '            raise RuntimeError("Command EVE prompt admission session unavailable")',
    '        db = manager._get_db() if callable(getattr(manager, "_get_db", None)) else None',
    '        if _command_eve_bind_claim_quarantine_db(agent, session_id, db) is None:',
    '            raise RuntimeError("Command EVE claim quarantine persistence unavailable")',
    '        agent._command_eve_session_id = session_id',
    '        if correction:',
    '            agent._command_eve_quarantine_reason = "user_correction"',
    '            if not _command_eve_persist_claim_quarantine(',
    '                agent, session_id, status="active", reason="user_correction",',
    '                turn_id=str(getattr(agent, "_command_eve_current_turn_id", "") or ""), evidence_digest=""',
    '            ):',
    '                raise RuntimeError("Command EVE claim quarantine persistence unavailable")',
    '            agent._command_eve_current_turn_has_correction = True',
    '            _command_eve_set_turn_memory_quarantine(session_id, True)',
    '        if raw_admission is None:',
    '            return await original_prompt(self, *args, **kwargs)',
    '        admission = command_eve_validate_prompt_admission(raw_admission)',
    '        agent._command_eve_current_turn_id = admission["turn_id"]',
    '        agent._command_eve_current_turn_has_attachment = True',
    '        if not _command_eve_persist_attachment_quarantine(agent, session_id, admission):',
    '            raise RuntimeError("Command EVE attachment quarantine persistence unavailable")',
    '        agent._command_eve_pending_prompt_admission = admission',
    '        agent._command_eve_prompt_admission_consumed = False',
    '        agent._command_eve_prompt_admission_conn = conn',
    '        agent._command_eve_prompt_admission_loop = asyncio.get_running_loop()',
    '        try:',
    '            return await original_prompt(self, *args, **kwargs)',
    '        finally:',
    '            pending = getattr(agent, "_command_eve_pending_prompt_admission", None)',
    '            consumed = bool(getattr(agent, "_command_eve_prompt_admission_consumed", False))',
    '            if pending is admission and not consumed:',
    '                # A slash/refusal/queue path never reached run_conversation, so',
    '                # admission was never verified. Keep the durable pending',
    '                # record active across crash/restart and leave the turn retryable.',
    '                agent._command_eve_pending_prompt_admission = None',
    '',
    '    def command_eve_run_conversation(self: Any, *args: Any, **kwargs: Any) -> Any:',
    '        admission = getattr(self, "_command_eve_pending_prompt_admission", None)',
    '        if admission is None:',
    '            return original_run(self, *args, **kwargs)',
    '        session_id = str(kwargs.get("task_id") or getattr(self, "_command_eve_session_id", "") or "").strip()',
    '        if session_id != str(getattr(self, "_command_eve_session_id", "") or "").strip():',
    '            raise RuntimeError("Command EVE prompt admission session mismatch")',
    '        conn = getattr(self, "_command_eve_prompt_admission_conn", None)',
    '        loop = getattr(self, "_command_eve_prompt_admission_loop", None)',
    '        if conn is None or loop is None:',
    '            raise RuntimeError("Command EVE prompt admission transport unavailable")',
    '        self._command_eve_pending_prompt_admission = None',
    '        self._command_eve_prompt_admission_consumed = True',
    '        def command_eve_admission_phase(phase: str, timeout_seconds: int | None) -> None:',
    '            params = dict(admission)',
    '            params["session_id"] = session_id',
    '            params["phase"] = phase',
    '            future = asyncio.run_coroutine_threadsafe(',
    '                conn.ext_method("command_eve/prompt_admission", params), loop',
    '            )',
    '            try:',
    '                response = future.result(timeout=timeout_seconds)',
    '            except Exception as exc:',
    '                future.cancel()',
    '                raise RuntimeError(f"Command EVE prompt admission {phase} failed: {exc}") from exc',
    '            expected = {"version", "request_id", "status"}',
    '            if (',
    '                not isinstance(response, dict)',
    '                or set(response) != expected',
    '                or response.get("version") != "command-eve-prompt-admission/v1"',
    '                or response.get("request_id") != admission["request_id"]',
    '                or response.get("status") != "accepted"',
    '            ):',
    '                raise RuntimeError(f"Command EVE prompt admission {phase} rejected")',
    ...
    '        return original_run(self, *args, **kwargs)',
    '',
    '    command_eve_prompt._command_eve_prompt_admission = True',
    '    command_eve_run_conversation._command_eve_prompt_admission = True',
    '    HermesACPAgent.prompt = command_eve_prompt',
    '    AIAgent.run_conversation = command_eve_run_conversation',
    '    HermesACPAgent._command_eve_prompt_admission_patch_installed = True',
    '    _command_eve_mark_patch("prompt_admission")',
    '',
    '',
    'def _require_command_eve_prompt_admission_patch() -> None:',
    '    try:',
    '        from acp_adapter.server import HermesACPAgent',
    '        from run_agent import AIAgent',
    '    except Exception as exc:',
    '        raise RuntimeError("Command EVE prompt admission seam unavailable") from exc',
    '    prompt = getattr(HermesACPAgent, "prompt", None)',
    '    drain = getattr(HermesACPAgent, "_drain_queued_prompts", None)',
    '    run = getattr(AIAgent, "run_conversation", None)',
    '    if not (',
    '        getattr(HermesACPAgent, "_command_eve_prompt_admission_patch_installed", False)',
    '        and callable(prompt)',
    '        and getattr(prompt, "_command_eve_prompt_admission", False)',
    '        and callable(drain)',
    '        and getattr(drain, "_command_eve_correction_fence", False)',
    '        and callable(run)',
    '        and getattr(run, "_command_eve_prompt_admission", False)',
    '    ):',
    '        raise RuntimeError("Command EVE prompt admission patch unavailable")',
    '',
    '',
    ...
    '# session_id -> the session cwd, recorded by the public prompt wrapper on',
    '# every real ACP turn before permission callbacks can run. The legacy',
    '# terminal-sync replacement below also records it if an older wheel calls it.',
    ...
    '        reason = "authority_timeout" if isinstance(exc, TimeoutError) or isinstance(nested_reason, TimeoutError) else "authority_unavailable"',
    '        return {"decision": "ask", "ladder": 0, "reason": reason}',
    '    decision = str(payload.get("decision") or "ask")',
    '    if decision not in {"allow", "ask"}:',
    '        decision = "ask"',
    '    try:',
    ...
    '    skill_write = normalized_tool == "skill_manage" and action not in {"list", "read", "show", "view"}',
    '    if (normalized_tool == "memory" or skill_write) and _command_eve_turn_memory_quarantined(session_id):',
    '        return {',
    '            "action": "block",',
    '            "message": "Command EVE quarantined memory and skill writes for this attachment or correction turn.",',
    '            "rule_key": "command-eve:grounding-quarantine",',
    '        }',
    '    authority = _command_eve_ask_tool_authority(normalized_tool, action)',
    '    if authority["decision"] == "allow":',
    '        return None',
    '    label = f"{normalized_tool}:{action}" if action else normalized_tool',
    '    reason = str(authority.get("reason") or "")',
    '    if reason == "authority_timeout":',
    '        prompt = f"Command EVE could not confirm the current authority grant within 2 seconds, so it requires one-operation approval for {label}."',
    '    else:',
    '        prompt = f"Command EVE requires your approval for {label} at this authority level."',
    '    try:',
    '        from tools import terminal_tool as _terminal_tool',
    '        approval_callback = _terminal_tool._get_approval_callback()',
    '    except Exception:',
    '        approval_callback = None',
    '    if not callable(approval_callback):',
    '        return {',
    '            "action": "block",',
    '            "message": f"BLOCKED: {prompt} No ACP approval callback is attached; the action was not queued.",',
    '            "rule_key": f"command-eve:{label}",',
    '        }',
    '    try:',
    '        choice = approval_callback(',
    '            f"<{normalized_tool}> {action}".rstrip(),',
    '            prompt,',
    ...
    '        return {',
    '            "action": "block",',
    '            "message": f"BLOCKED: Command EVE could not deliver the ACP approval request for {label}.",',
    '            "rule_key": f"command-eve:{label}",',
    '        }',
    '    if choice == "once":',
    '        return None',
    '    outcome = "timed out" if choice == "timeout" else "was denied"',
    '    return {',
    '        "action": "block",',
    '        "message": f"BLOCKED: Command EVE approval for {label} {outcome}.",',
    '        "rule_key": f"command-eve:{label}",',
    '    }',
    '',
    '',
    ...
    '        factory for factory in (server_factory, module_factory)',
    '        if callable(factory) and getattr(factory, "_command_eve_authority_patch", False)',
    '    ), None)',
    '    if patched_factory is not None:',
    '        acp_permissions.make_approval_callback = patched_factory',
    '        acp_server.make_approval_callback = patched_factory',
    '        if acp_server.make_approval_callback is not acp_permissions.make_approval_callback:',
    '            raise RuntimeError("Command EVE approval factory binding diverged")',
    '        _command_eve_mark_patch("approval_class")',
    '        return',
    '    # Hermes imports this symbol into acp_adapter.server at module load. Wrap',
    '    # the factory the server ACTUALLY calls, not only the later module value.',
    '    original_factory = server_factory if callable(server_factory) else module_factory',
    ...
    '    acp_server.make_approval_callback = command_eve_make_approval_callback',
    '    if acp_server.make_approval_callback is not acp_permissions.make_approval_callback:',
    '        raise RuntimeError("Command EVE approval factory binding diverged")',
    '    _command_eve_mark_patch("approval_class")',
    '',
    ...
    '        # unrelated surfaces (V22 tools/computer_use/tool.py gates on the',
    '        # session approval-bypass state), so flipping it would silently widen far',
    ...
    '_COMMAND_EVE_PROVIDER_TURN_BINDINGS_SENT: set[tuple[str, str]] = set()',
    '# Hermes 0.20 exposes the native clarify tool to ACP but never supplies its',
    '# platform callback (FACT whl:acp_adapter/server.py::_prompt_impl only binds',
    '# approval_cb). Keep this compatibility seam version-bound and delete it once',
    ...
    ') -> str:',
    '    """Bridge one native Hermes 0.20 single-choice clarify request to ACP."""',
    ...
    '',
    '    if multi_select:',
    '        raise RuntimeError("Command EVE ACP clarify does not support multi-select")',
    '    if not isinstance(choices, list) or not 1 <= len(choices) <= 4:',
    '        raise RuntimeError("Command EVE ACP clarify requires one to four visible choices")',
    '    normalized_choices: list[str] = []',
    '    for choice in choices:',
    '        if not isinstance(choice, str) or not choice.strip() or len(choice.strip()) > 512:',
    ...
    '    if not callable(request_permission) or loop is None:',
    '        raise RuntimeError("Command EVE ACP clarify has no permission bridge")',
    '    if threading.get_ident() == loop_thread_id:',
    '        raise RuntimeError("Command EVE ACP clarify cannot block the ACP event loop")',
    '',
    '    options = [',
    '        PermissionOption(option_id=f"clarify_choice_{index}", kind="allow_once", name=choice)',
    ...
    '        kind="execute",',
    '        status="pending",',
    '        content=[acp.tool_content(acp.text_block(question_text))],',
    '        raw_input={',
    '            "question": question_text,',
    '            "choices": list(normalized_choices),',
    '            "metadata": metadata,',
    ...
    '        request_permission(session_id=session_id, tool_call=tool_call, options=options),',
    '        loop,',
    '        logger=logging.getLogger(__name__),',
    '        log_message="Command EVE ACP clarify failed to schedule",',
    '    )',
    '    if future is None:',
    '        raise RuntimeError("Command EVE ACP clarify could not be scheduled")',
    '    try:',
    '        response = future.result(timeout=_COMMAND_EVE_ACP_CLARIFY_TIMEOUT_SECONDS)',
    '    except FutureTimeout as exc:',
    '        future.cancel()',
    '        raise RuntimeError("Command EVE ACP clarify timed out") from exc',
    '    except Exception as exc:',
    '        future.cancel()',
    '        raise RuntimeError("Command EVE ACP clarify failed") from exc',
    '    outcome = getattr(response, "outcome", None)',
    '    if not isinstance(outcome, AllowedOutcome):',
    '        raise RuntimeError("Command EVE ACP clarify was cancelled")',
    '    option_id = str(getattr(outcome, "option_id", "") or "")',
    '    prefix = "clarify_choice_"',
    '    if not option_id.startswith(prefix):',
    '        raise RuntimeError("Command EVE ACP clarify returned an unknown choice")',
    '    try:',
    '        selected_index = int(option_id[len(prefix):])',
    '        return normalized_choices[selected_index]',
    ...
    '            pass',
    '',
    '    # PROGRESSIVE DISCLOSURE, AIMED AT THE SURFACE IT WAS BUILT FOR.',
    '    #',
    '    # Hermes defers every non-core tool behind tool_search/tool_describe/',
    '    # tool_call the moment ONE deferrable tool exists (FACT wheel',
    '    # tools/tool_search.py should_activate: any deferrable tool activates;',
    '    # threshold_pct only sizes the LISTING, not the activation). That rule',
    '    # was written for the case it names in its own docstring: a flat',
    '    # third-party API surface with thousands of tools.',
    '    #',
    ...
    '    # code-exec, delegation) eager — the exact inverse of its purpose.',
    '    #',
    '    # The cost is not the ~2.2K tokens it saves. It is that reaching an',
    '    # app-owned tool takes tool_search -> tool_describe -> tool_call before',
    '    # the real call, and on the EVE cloud lane every one of those rounds is',
    '    # a FULLY BUFFERED wait: the edge function reads the whole SSE document,',
    '    # commits the debit, and only then releases a byte (FACT Company.OS',
    '    # supabase/functions/_shared/eve-inference-core.ts, 1.820.1 — "only a',
    '    # COMMITTED debit releases the bytes"). Extra rounds are extra silent',
    '    # windows, each re-uploading the full context.',
    '    #',
    ...
    '    # hermes-cron, hermes-telegram, hermes-slack, hermes-signal and the rest',
    ...
    '    # hermes_tool_search_disclosure_harness.py.',
    '    #',
    '    # SCOPE, STATED HONESTLY: this covers the tools THIS app owns and ships.',
    '    # A user plugin that registers its own platform toolset resolves through',
    '    # a separate auto-generate branch that reads the same lazy name and would',
    '    # see these names too (wheel toolsets.py resolve_toolset). Command EVE',
    '    # emits no such platform, so nothing here triggers it today.',
    '    command_eve_always_visible = (',
    '        "eve_artifact_get",',
    '        "eve_artifact_list",',
    ...
    '    )',
    '    try:',
    '        existing_core = list(getattr(toolsets, "_HERMES_CORE_TOOLS", ()) or ())',
    '        missing_core = [n for n in command_eve_always_visible if n not in existing_core]',
    '        if missing_core:',
    '            # A NEW list object. Never .append()/.extend() — see above.',
    '            toolsets._HERMES_CORE_TOOLS = existing_core + missing_core',
    '            try:',
    '                import model_tools',
    '                clear_tool_cache = getattr(model_tools, "_clear_tool_defs_cache", None)',
    '                if callable(clear_tool_cache):',
    ...
    '                pass',
    '    except Exception:',
    '        # Disclosure is an optimisation. A failure here must never cost the',
    '        # seat its desktop bridge, so the patch continues either way.',
    '        pass',
    ...
    '        return',
    '',
    '    # desktop_ui owns one process-global emitter. Bind it once to the',
    '    # task-local session key and resolve the live ACP connection through the',
    '    # existing owner-fenced map; a per-turn closure would let a concurrent',
    "    # turn replace another session's renderer route.",
    ...
    '        with _COMMAND_EVE_DESKTOP_CONNECTIONS_LOCK:',
    '            bridge = _COMMAND_EVE_DESKTOP_CONNECTIONS.get(active_session_id)',
    '        if bridge is None:',
    '            raise RuntimeError("desktop event has no active Command EVE turn")',
    '        conn, loop, loop_thread_id, _owner = bridge',
    ...
    '        prompt_quarantine_agent = None',
    '        runtime_status_agent = None',
    '        previous_runtime_status_callback = None',
    ...
    '                prompt_quarantine_agent = turn_agent',
    '                quarantine_db = manager._get_db() if callable(getattr(manager, "_get_db", None)) else None',
    '                _command_eve_bind_claim_quarantine_db(turn_agent, session_id, quarantine_db)',
    '                _command_eve_enter_turn_memory_quarantine(turn_agent, session_id, prompt_blocks)',
    '        except Exception:',
    '            logging.getLogger(__name__).warning(',
    ...
    '                    runtime_status_agent = agent',
    '                    previous_runtime_status_callback = getattr(',
    '                        agent, "command_eve_runtime_status_callback", None',
    '                    )',
    '',
    '                    def command_eve_runtime_status(payload: dict[str, Any]) -> None:',
    '                        active_session_id = str(get_session_env("HERMES_SESSION_KEY", "") or "").strip()',
    '                        if not active_session_id or (session_id and active_session_id != session_id):',
    '                            return',
    '                        phase = payload.get("phase") if isinstance(payload, dict) else None',
    '                        if phase not in {"provider_wait", "retry_wait"}:',
    '                            return',
    ...
    '                                or not (1 <= attempt <= max_attempts <= 100)',
    '                                or not (0 <= retry_after_ms <= 600_000)',
    '                            ):',
    '                                return',
    '                            envelope.update({',
    '                                "attempt": attempt,',
    ...
    '                        update = SessionInfoUpdate(',
    '                            session_update="session_info_update",',
    '                            field_meta={"commandEveRuntimeStatus": envelope},',
    '                        )',
    '                        delivery = conn.session_update(active_session_id, update)',
    '                        if threading.get_ident() == loop_thread_id:',
    '                            loop.create_task(delivery)',
    '                            return',
    '                        asyncio.run_coroutine_threadsafe(delivery, loop)',
    '',
    '                    agent.command_eve_runtime_status_callback = command_eve_runtime_status',
    '            except Exception:',
    '                logging.getLogger(__name__).warning(',
    ...
    '            return await original_prompt(self, *args, **kwargs)',
    '        finally:',
    '            if prompt_quarantine_agent is not None:',
    '                try:',
    '                    _command_eve_exit_turn_memory_quarantine(prompt_quarantine_agent, session_id)',
    '                except Exception:',
    '                    logging.getLogger(__name__).warning(',
    '                        "Command EVE could not clear attachment turn", exc_info=True',
    '                    )',
    ...
    '                except Exception:',
    '                    pass',
    '            if runtime_status_agent is not None:',
    '                try:',
    '                    runtime_status_agent.command_eve_runtime_status_callback = previous_runtime_status_callback',
    '                except Exception:',
    '                    pass',
    '            if session_id:',
    '                with _COMMAND_EVE_DESKTOP_CONNECTIONS_LOCK:',
    ...
    '                        # MATERIALISE before mutating. A generator argument here',
    '                        # reads the SAME set difference_update is shrinking, which',
    '                        # raises RuntimeError("Set changed size during iteration")',
    '                        # on every turn that emitted at least one provider binding',
    '                        # (CEVE-18232). The turn itself had already succeeded, so',
    '                        # the failure surfaced as a red card AFTER a good answer',
    '                        # and evicted the ACP agent, costing a ~10s cold start on',
    '                        # the next turn. The list comprehension is fully consumed',
    '                        # before the first element is removed.',
    '                        _COMMAND_EVE_PROVIDER_TURN_BINDINGS_SENT.difference_update(',
    '                            [',
    '                                binding',
    '                                for binding in _COMMAND_EVE_PROVIDER_TURN_BINDINGS_SENT',
    '                                if binding[0] == session_id',
    '                            ]',
    '                        )',
    '',
    ...
    '',
    '',
    'def _install_command_eve_runtime_status_patch() -> None:',
    '    """Project provider wait/retry as content-free ACP metadata.',
    '',
    '    Hermes intentionally keeps local wait strings out of ACP thought panes.',
    '    This adapter preserves that boundary: it never forwards those strings,',
    '    only a closed structured phase plus bounded retry numbers.',
    '    """',
    '    try:',
    '        from run_agent import AIAgent',
    '    except Exception:',
    '        return',
    '    if getattr(AIAgent, "_command_eve_runtime_status_patch_installed", False):',
    '        _command_eve_mark_patch("runtime_status")',
    '        return',
    '    original_wait = getattr(AIAgent, "_emit_wait_notice", None)',
    '    original_buffer = getattr(AIAgent, "_buffer_status", None)',
    '    if not callable(original_wait) or not callable(original_buffer):',
    '        return',
    '',
    '    def command_eve_emit_runtime_status(agent: Any, payload: dict[str, Any]) -> None:',
    '        callback = getattr(agent, "command_eve_runtime_status_callback", None)',
    ...
    '            if 1 <= attempt <= max_attempts <= 100:',
    '                command_eve_emit_runtime_status(self, {',
    '                    "phase": "retry_wait",',
    '                    "attempt": attempt,',
    '                    "maxAttempts": max_attempts,',
    '                    "retryAfterMs": retry_after_ms,',
    ...
    '        return result',
    '',
    '    AIAgent._emit_wait_notice = command_eve_wait_notice',
    '    AIAgent._buffer_status = command_eve_buffer_status',
    '    AIAgent._command_eve_runtime_status_patch_installed = True',
    '    _command_eve_mark_patch("runtime_status")',
    '',
    '',
    'def _install_command_eve_acp_clarify_patch() -> None:',
    '    """Supply the native Hermes 0.20 clarify callback on ACP turns only."""',
    '    try:',
    '        from acp_adapter import server as acp_server',
    '        from gateway.session_context import get_session_env',
    '        from tools.registry import registry',
    '    except Exception:',
    '        return',
    '    if str(getattr(acp_server, "HERMES_VERSION", "") or "") != "0.20.0":',
    '        return',
    '    # Hermes 0.20 advertises open-ended and multi-select clarify shapes, but',
    '    # this ACP permission transport is intentionally single-select. Narrow the',
    '    # model-visible schema to what the version-bound callback can render.',
    '    clarify_entry = registry.get_entry("clarify")',
    '    if clarify_entry is not None:',
    '        clarify_entry.schema = {',
    '            "name": "clarify",',
    '            "description": (',
    '                "Ask the operator one structured single-select question when a real ambiguity, "',
    '                "decision, feedback choice, or meaningful trade-off blocks useful progress. "',
    '                "Provide one to four short choices. Prefer a safe reversible default for low-stakes decisions; "',
    '                "dangerous commands keep their dedicated security approval card."',
    '            ),',
    '            "parameters": {',
    '                "type": "object",',
    '                "properties": {',
    '                    "question": {',
    '                        "type": "string",',
    '                        "description": "The visible question only; put every selectable answer in choices.",',
    '                    },',
    '                    "choices": {',
    '                        "type": "array",',
    '                        "items": {"type": "string"},',
    '                        "minItems": 1,',
    '                        "maxItems": 4,',
    '                        "description": "One to four short single-select answers rendered as buttons.",',
    '                    },',
    '                },',
    ...
    '        except Exception:',
    '            pass',
    '    HermesACPAgent = acp_server.HermesACPAgent',
    '    if getattr(HermesACPAgent, "_command_eve_acp_clarify_patch_installed", False):',
    ...
    '        return',
    '',
    '    async def command_eve_clarify_prompt(self: Any, *args: Any, **kwargs: Any) -> Any:',
    '        session_arg = kwargs.get("session_id")',
    '        if session_arg is None and len(args) > 1:',
    '            session_arg = args[1]',
    '        canonical_session_id = str(session_arg or "").strip()',
    ...
    '',
    '',
    '# ── CEVE-18232: the epilogue must not un-answer an answered turn ──────────',
    '#',
    '# acp/connection.py::_run_request wraps ANY exception escaping a request into',
    '# RequestError.internal_error and — critically — re-raises it with `from None`.',
    '# The originating traceback is therefore never printed, which is why one',
    '# `RuntimeError("Set changed size during iteration")` cost days of guessing.',
    '# It is not lost though: `raise X from None` only sets __suppress_context__,',
    '# the original exception still hangs on __context__ WITH its traceback.',
    '# The handler below prints exactly that, and changes no behaviour at all.',
    '_COMMAND_EVE_TURN_COMPLETION_LOCK = threading.Lock()',
    '_COMMAND_EVE_TURN_COMPLETION: dict[str, int] = {}',
    '_COMMAND_EVE_TURN_COMPLETION_MAX = 64',
    '',
    '',
    'class _CommandEveInternalErrorContextHandler(logging.Handler):',
    '    """Print the cause that acp\'s ``raise err from None`` hides."""',
    '',
    '    def emit(self, record: Any) -> None:',
    '        import sys',
    '        import traceback',
    ...
    '            if not exc_info:',
    '                return',
    '            cause = getattr(exc_info[1], "__context__", None)',
    '            if cause is None:',
    '                return',
    '            rendered = "".join(',
    ...
    '            )',
    '            sys.stderr.write(',
    '                "Command EVE: ACP internal error — ORIGINAL cause below "',
    '                "(acp re-raised it with `from None`, hiding this):\\n" + rendered',
    '            )',
    '        except Exception:',
    '            # A diagnostic channel may never take down the process it observes.',
    '            return',
    '',
    '',
    'def _install_command_eve_acp_internal_error_context_patch() -> None:',
    '    """Attach the cause printer to the root logger.',
    '',
    '    Root, not a named logger: acp calls the MODULE-level ``logging.exception``,',
    '    which is the root logger by definition. Patching Connection._on_task_error',
    '    instead would be too late — it is bound into the handler list in',
    '    Connection.__init__, long before a provider call reaches this shim.',
    '    """',
    '    root = logging.getLogger()',
    '    for existing in root.handlers:',
    '        if isinstance(existing, _CommandEveInternalErrorContextHandler):',
    '            _command_eve_mark_patch("acp_internal_error_context")',
    '            return',
    '    handler = _CommandEveInternalErrorContextHandler()',
    '    handler.setLevel(logging.ERROR)',
    '    root.addHandler(handler)',
    '    _command_eve_mark_patch("acp_internal_error_context")',
    '',
    '',
    'class _CommandEveTurnCompletionHandler(logging.Handler):',
    ...
    '        try:',
    '            message = record.getMessage()',
    '            if not message.startswith("Turn ended:"):',
    '                return',
    '            match = re.search(r"session=(\\S+)", message)',
    '            if match is None:',
    '                return',
    '            session_id = match.group(1)',
    '            with _COMMAND_EVE_TURN_COMPLETION_LOCK:',
    '                if (',
    '                    session_id not in _COMMAND_EVE_TURN_COMPLETION',
    '                    and len(_COMMAND_EVE_TURN_COMPLETION) >= _COMMAND_EVE_TURN_COMPLETION_MAX',
    '                ):',
    '                    _COMMAND_EVE_TURN_COMPLETION.pop(',
    '                        next(iter(_COMMAND_EVE_TURN_COMPLETION)), None',
    '                    )',
    '                _COMMAND_EVE_TURN_COMPLETION[session_id] = (',
    '                    int(_COMMAND_EVE_TURN_COMPLETION.get(session_id, 0)) + 1',
    '                )',
    '        except Exception:',
    '            return',
    '',
    '',
    'def _command_eve_attach_turn_completion_capture() -> None:',
    '    logger = logging.getLogger("agent.conversation_loop")',
    '    for existing in logger.handlers:',
    '        if isinstance(existing, _CommandEveTurnCompletionHandler):',
    '            return',
    '    handler = _CommandEveTurnCompletionHandler()',
    '    handler.setLevel(logging.INFO)',
    '    logger.addHandler(handler)',
    '',
    '',
    'def _command_eve_turn_completions(session_id: str) -> int:',
    '    with _COMMAND_EVE_TURN_COMPLETION_LOCK:',
    '        return int(_COMMAND_EVE_TURN_COMPLETION.get(session_id, 0))',
    '',
    '',
    'def _install_command_eve_acp_epilogue_isolation_patch() -> None:',
    '    """A turn that reached its end is ANSWERED. Its epilogue cannot un-answer it.',
    '',
    '    After ``Turn ended`` the ACP adapter still persists history, reconciles the',
    '    heartbeat, judges the goal, drains the queue and reports usage. Those are',
    '    bookkeeping. When one of them raised, the exception left prompt(), acp turned',
    '    it into a -32603 response, AionCore marked the agent unhealthy and evicted it',
    '    — so a correct, already-rendered answer became a red card and the NEXT turn',
    '    paid a ~10s cold start. That is the defect, independent of which bookkeeping',
    '    step broke.',
    '',
    '    THIS IS NOT A BLANKET SWALLOW. Three deliberate boundaries:',
    '',
    '    1. PROOF REQUIRED. Containment happens only when the agent loop logged',
    '       ``Turn ended`` FOR THIS SESSION during THIS call. Without that proof the',
    '       exception propagates exactly as before, so every pre-turn and in-turn',
    '       failure keeps its current reporting.',
    '    2. BaseException IS NOT CAUGHT. CancelledError, KeyboardInterrupt and',
    '       SystemExit are not ``Exception`` in Python 3.8+, so cancellation keeps',
    '       working untouched.',
    ...
    '    ``save_session``: a failed persist cannot invalidate the turn. The adapter',
    '    assigns ``state.history`` BEFORE persisting, and the next turn builds on that',
    '    in-memory history, so a persist failure costs durability across a process',
    '    restart, never correctness of the following turn. It also cannot reach this',
    '    wrapper: SessionManager._persist already catches Exception internally and',
    '    only warns. If a future epilogue step ever does invalidate the turn, it',
    '    belongs here as a named exemption — not as a silent one.',
    '',
    '    The failure is logged at ERROR with the full traceback either way.',
    '    """',
    '    try:',
    '        from acp_adapter.server import HermesACPAgent',
    '    except Exception:',
    '        return',
    '    if getattr(HermesACPAgent, "_command_eve_acp_epilogue_isolation_patch_installed", False):',
    '        _command_eve_mark_patch("acp_epilogue_isolation")',
    '        return',
    '    original_prompt = getattr(HermesACPAgent, "_prompt_impl", None)',
    '    if not callable(original_prompt):',
    '        return',
    ...
    '',
    '    async def command_eve_epilogue_isolated_prompt(self: Any, *args: Any, **kwargs: Any) -> Any:',
    '        _command_eve_attach_turn_completion_capture()',
    '        session_id = kwargs.get("session_id")',
    '        if not session_id and len(args) >= 2:',
    '            session_id = args[1]',
    '        session_id = str(session_id or "")',
    '        completed_before = _command_eve_turn_completions(session_id)',
    '        try:',
    '            return await original_prompt(self, *args, **kwargs)',
    '        except Exception as exc:',
    '            try:',
    ...
    '            except Exception:',
    '                RequestError = None',
    '            if RequestError is not None and isinstance(exc, RequestError):',
    '                raise',
    '            if not session_id:',
    '                raise',
    '            if _command_eve_turn_completions(session_id) <= completed_before:',
    '                raise',
    '            logging.getLogger(__name__).error(',
    '                "Command EVE: ACP turn epilogue failed AFTER the turn completed "',
    '                "for session %s. The delivered answer stands and the agent stays "',
    '                "healthy; the bookkeeping step below did not run.",',
    '                session_id,',
    '                exc_info=True,',
    '            )',
    '            from acp.schema import PromptResponse',
    ...
    '            return PromptResponse(stop_reason="end_turn")',
    '',
    '    HermesACPAgent._prompt_impl = command_eve_epilogue_isolated_prompt',
    '    HermesACPAgent._command_eve_acp_epilogue_isolation_patch_installed = True',
    '    _command_eve_mark_patch("acp_epilogue_isolation")',
    '',
    '',
    ...
    '        # end_turn is a transport completion signal, not evidence that a',
    '        # corrected false claim disappeared. Durable correction quarantine',
    '        # remains active until an explicit typed reconcile receipt is applied.',
    ...
    '        # LAST ON PURPOSE. Each _prompt_impl installer wraps the previous one, so',
    '        # the one installed last is OUTERMOST. The epilogue guard has to sit',
    '        # outside desktop_bridge/clarify/session_guard, because their own finally',
    '        # blocks are part of the epilogue it contains — CEVE-18232 raised in',
    '        # exactly such a finally block.',
    ...
    '_install_command_eve_acp_disabled_toolsets_patch()',
    '_install_command_eve_acp_internal_error_context_patch()',
    '# LAST at import too — same outermost-wrapper reasoning as in the retry list.',
    '_install_command_eve_acp_epilogue_isolation_patch()',
    '',
    ...
    '            or not getattr(_ce_approval_factory, "_command_eve_authority_patch", False)',
...
function writeHermesRuntimeFiles(
  ...
  // admission repair then re-establishes the packaged wrappers.
  packagedArtifactSite?: Extract<CommandEveArtifactPythonSiteVerification, { ok: true }>
): string[] {
  ...
  const config = [
    ...
    // Do not let Hermes replay an unchanged oversized prompt against Supabase's
    ...
    // Keep the percentage for compatibility, but use Hermes' native absolute
    // threshold as the real latency guard. Hermes raises percentages below 0.75
    // on sub-512K models, which made 0.75 wait until ~196K on EVE's 256K lane.
    //
    // Full compression remains a fail-safe fallback: 48k projects to ~76s at the
    // measured 634 tokens/s, while its cloud budget intentionally stays 14s/29s.
    // The normal path is Hermes' existing deterministic, no-LLM pruning seam,
    // which summarizes old tool results toward 32k before the foreground call.
    //
    // Residual 1.824.0 debt: configuration alone cannot guarantee TTFT <150s.
    // Even ~18.5k-token prompts measured 80-146s; the ~43KB per-turn tool-schema
    // baseline and provider cold start still require progressive tool disclosure.
    'compression:',
    `  threshold: ${COMMAND_EVE_CONTEXT_COMPRESSION_THRESHOLD.toFixed(2)}`,
    `  threshold_tokens: ${DEFAULT_COMMAND_EVE_COMPRESSION_THRESHOLD_TOKENS}`,
    `  target_ratio: ${DEFAULT_COMMAND_EVE_COMPRESSION_TARGET_RATIO.toFixed(2)}`,
    `  proactive_prune_tokens: ${DEFAULT_COMMAND_EVE_PROACTIVE_PRUNE_TOKENS}`,
    `  proactive_prune_min_result_chars: ${DEFAULT_COMMAND_EVE_PROACTIVE_PRUNE_MIN_RESULT_CHARS}`,
    `  proactive_prune_min_reclaim_tokens: ${DEFAULT_COMMAND_EVE_PROACTIVE_PRUNE_MIN_RECLAIM_TOKENS}`,
  ...
  writeHermesCommandWrapper(paths, packagedArtifactSite);
  writeHermesCliShim(paths, packagedArtifactSite);
...
async function ensureCommandEveRuntimeBootstrapUnlocked(
  ...
  let selectedPythonSha256: string | undefined;
  try {
    selectedPythonSha256 = sha256FileIfPresent(python.path);
  } catch {
    selectedPythonSha256 = undefined;
  }
  runtimeProvenance.python = {
    executable: python.path,
    ...(selectedPythonSha256 ? { executable_sha256: selectedPythonSha256 } : {}),
    version: python.version || 'unknown',
    source: requireBundledPython
      ? 'bundled'
      : pythonUsesBundledCandidate
  ...
  if (packagedHermesSiteRequired && !artifactSite.ok) {
    ...
    pushStage(
      makeStage('presentation-python', mode === 'check' ? 'blocked' : 'failed', {
        code: 'HERMES_PACKAGED_DEPENDENCY_SITE_INVALID',
  ...
  // Prove the incoming wheel before consulting any installed-version state or
  // mutating signed-site bindings. A missing/tampered release artifact must
  // fail without letting a plausible legacy environment influence the gate.
  ...
  runtimeProvenance.hermes = {
    ...
    dependency_resolution: requireBundledPython ? 'packaged_signed_site' : 'pypi_tls_on_first_boot',
  ...
  // venv still owns import precedence. Once the signed-site .pth and native
  // launchers are written, importlib.metadata truthfully reports the packaged
  // version instead; using that later value to decide whether to back up
  // state.db would silently skip a real version crossing.
  if (
    mode !== 'check' &&
    !existingVenvNeedsReplacement &&
    ...
    if (!priorVersion) {
      pushStage(
        makeStage('hermes', 'failed', {
          code: 'HERMES_PRIOR_VERSION_UNPROVEN',
          detail:
  ...
  let packagedRuntimeProbe: RuntimeBootstrapCommandResult | undefined;
  if (packagedHermesSiteRequired && artifactSite.ok) {
    // Verify the complete signed package set and every distribution/module
    // origin BEFORE writing a .pth file or launcher into the mutable venv.
    // This runs even for a same-version reused venv so compatible legacy
    // packages can never shadow the signed runtime while the receipt claims
    // packaged_signed_site.
    const packagedProbeArgs = packagedHermesRuntimeProbeArgs(manifest.hermes.version, artifactSite);
    packagedRuntimeProbe = await runner(pythonBinary(paths), packagedProbeArgs, {
      env,
      timeoutMs: DEFAULT_STAGE_TIMEOUT_MS,
    });
    if (!packagedHermesRuntimeProbePassed(packagedRuntimeProbe)) {
      ...
      pushStage(
        makeStage('hermes', mode === 'check' ? 'blocked' : 'failed', {
          code: 'HERMES_PACKAGED_DEPENDENCY_ORIGIN_INVALID',
          detail,
  ...
  if (packagedHermesSiteRequired && artifactSite.ok && mode !== 'check') {
    const bindingResult = await bindCommandEveArtifactPythonSite({
      paths,
      artifactSite: artifactSite.directory,
      runner,
      env,
    });
    pathBinding = bindingResult;
    if (!bindingResult.ok) {
      const bindingReason = 'reason' in bindingResult ? bindingResult.reason : 'artifact_site_bind_failed';
      const detail = rollbackVenvReplacement(
        `The packaged Hermes dependency closure could not be bound (${bindingReason}).`
      );
      pushStage(
        makeStage('presentation-python', 'failed', {
          code: 'HERMES_PACKAGED_DEPENDENCY_SITE_BIND_FAILED',
          detail,
        })
      );
      return commitReceipt();
  ...
  runtimeProvenance.hermes = {
    ...
    dependency_resolution: requireBundledPython ? 'packaged_signed_site' : 'pypi_tls_on_first_boot',
    ...(requireBundledPython && artifactSite.ok
      ? {
          signed_site: {
            path: artifactSite.directory,
            receipt_sha256: artifactSite.receiptSha256,
  ...
  if (mode === 'check') {
    ...
    // copied aside HERE, before the replacement touches the venv: once per seat, origin
    // and target version in the name, idempotent, missing DB = first run =
    // no-op. A failed copy or seat enumeration is fail-closed: continuing
    // would cross a one-way state schema without a recoverable origin. A
    ...
    if (hermesInstalled && Boolean(installedHermesVersion) && !hermesVersionMatches) {
      ...
      const failedBackups = stateDbBackups.filter((result) => result.status === 'failed');
      if (failedBackups.length > 0) {
        pushStage(
          makeStage('hermes', 'failed', {
            code: 'HERMES_STATE_DB_BACKUP_FAILED',
    ...
    let install: RuntimeBootstrapCommandResult;
    if (requireBundledPython) {
    ...
    const failureDetail = install.ok
      ? 'Hermes was verified, but its private wheel receipt could not be persisted.'
    ...
    pushStage(
      makeStage('hermes', hermesReady ? 'pass' : 'failed', {
        ...
        detail: hermesReady
          ? requireBundledPython
            ? `Verified ${manifest.hermes.package} ${manifest.hermes.version} from the signed offline dependency site.`
        ...
        command: requireBundledPython
          ? `${pythonBinary(paths)} -B -c <packaged-hermes-offline-probe>`
          : `${pythonBinary(paths)} ${installArgs.join(' ')}`,
        duration_ms: Date.now() - started,
      })
    );
    if (!hermesReady) {
  ...
  if (mode !== 'check') writeHermesCliShim(paths, artifactSite.ok ? artifactSite : undefined);
  ...
  // Strict packaged macOS already proved DDGS and its entire closure inside the
  // signed site. It must never mutate the admitted venv from PyPI. Development
  // keeps the legacy best-effort installer below.
  if (requireBundledPython) {
    pushStage(
      makeStage('web', 'pass', {
  ...
  } else if (mode !== 'check' && fs.existsSync(pythonBinary(paths))) {
    ...
    if (!ddgsProbe.ok) {
      const started = Date.now();
      // Development/unpackaged runs may use the vendored raw-wheel closure.
      ...
      // Any network fallback here is development-only; strict packaged macOS
      // returned through the signed-site branch above.
      ...
      // Surface the development fallback honestly in its receipt.
  ...
  const emitHermesRuntimeFiles = (): string[] =>
    writeHermesRuntimeFiles(
      ...
      localVisionModelRef,
      artifactSite.ok ? artifactSite : undefined
    );
