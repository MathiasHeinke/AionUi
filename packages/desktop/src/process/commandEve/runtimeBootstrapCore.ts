...
function writeHermesOllamaProviderOverride(paths: RuntimeBootstrapPaths): void {
  ...
  const initPy = [
    ...
    '                inside = _command_eve_command_inside_workspace(str(command or ""), cwd)',
    '                if _command_eve_ask_authority(str(command or ""), inside)["decision"] == "allow":',
    '                    return "once"',
    '            except Exception:',
    ...
    '        )',
    '',
    '    # One named toolset, three tools. The raw terminal desktop affordances and',
    '    # wheel kanban tools never enter the ACP selection list.',
    '    toolsets.TOOLSETS["command-eve-desktop"] = {',
    '        "description": "Command EVE bounded preview read/open and files-pane controls",',
    ...
    '        for tool_name in ("open_preview", "read_preview", "focus_pane"):',
    ...
    '    if getattr(_ce_agent, "_command_eve_permission_authority_patch_installed", False):',
    '        return True, ""',
    ...
    '        "model call while Hermes could route approvals itself instead of AionCore."',
    '    )',
    '',
    '',
