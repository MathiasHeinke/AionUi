...
export async function runHonchoBootstrap(deps: HonchoBootstrapDeps): Promise<HonchoBootstrapResult> {
  const plan: HonchoProvisionPlan = deps.plan || {};
  const config: HonchoRuntimeConfig = deps.config || {};
  const nowMs = (): number => (typeof deps.now === 'function' ? deps.now() : Date.now());
  ...
  const finalize = (readiness: HonchoReadinessState): HonchoBootstrapResult => {
    try {
      if (config.honchoHome && typeof deps.writeReadiness === 'function') {
  ...
  // (3) The readiness probe. If provisioning did not complete, do not even probe.
  if (!ok) {
    stages.push({ id: HONCHO_STEP_READY, status: 'skip', code: HONCHO_REASON_DEP_MISSING, detail: 'provisioning did not complete' });
