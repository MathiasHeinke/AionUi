...
function inferType(
  ...
  const pathOrUrl = readString(payload, ['path', 'file_path', 'absolute_path', 'url', 'file_url', 'href', 'src']);
...
const MessageGeneratedArtifact: React.FC<{ artifact: IGeneratedConversationArtifact }> = ({ artifact }) => {
  ...
  const path = readString(payload, ['path', 'file_path', 'absolute_path']);
  const source = readString(payload, ['url', 'file_url', 'href', 'src']) || (path ? pathToFileUrl(path) : undefined);
  const title =
