...
export const MIN_CHAT_PANEL_PX = 340;
...
export type AdaptiveWorkbenchLayoutMode = 'focus' | 'split-left' | 'split-right' | 'split-bottom';

/**
 * A 340px chat and a 340px work surface cannot coexist horizontally below
 * 680px. Switch to the already-supported stacked layout instead of letting
 * flexbox clip either pane or collapsing the composer into a two-line strip.
 * A width of 0 means the ResizeObserver has not measured the container yet.
 */
export const resolveAdaptiveWorkbenchLayout = (
  requested: AdaptiveWorkbenchLayoutMode,
  availableWidth: number
): AdaptiveWorkbenchLayoutMode => {
  const horizontal = requested === 'split-left' || requested === 'split-right';
  if (horizontal && availableWidth > 0 && availableWidth < MIN_HORIZONTAL_WORKBENCH_PX) return 'split-bottom';
  return requested;
};
...
export const calcLayoutMetrics = (input: LayoutCalcInput): LayoutMetrics => {
  ...
  const shouldConstrainChatPreview = workspaceEnabled && isDesktop && isPreviewOpen;
  const dynamicChatMinRatio = shouldConstrainChatPreview ? Math.max(MIN_CHAT_RATIO, minChatRatioByPx) : MIN_CHAT_RATIO;
  const dynamicChatMaxCandidate = shouldConstrainChatPreview
