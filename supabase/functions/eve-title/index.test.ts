...
import { handleEveTitle, resetEveTitlePublicKeyCacheForTests } from './index.ts';
...
function licenseWire(expiresAt: string | null = '2027-07-08T00:00:00.000Z'): string {
...
Deno.test('rejects expired license bearers conclusively', async () => {
  ...
  const response = await handleEveTitle(request({ text: 'Bitte plane den Launch' }, expiredWire));
  const body = await response.json();
...
Deno.test('rejects provider key fields in request bodies', async () => {
  const response = await handleEveTitle(request({ text: 'Hallo', metadata: { openrouterApiKey: 'leak' } }));
  const body = await response.json();
...
Deno.test('requires server-side OPENROUTER_API_KEY before provider execution', async () => {
  try {
    disableProvider();
    const response = await handleEveTitle(request({ text: 'Bitte plane den Launch', locale: 'de-DE' }));
    const body = await response.json();
...
Deno.test('maps provider failures without echoing provider response bodies', async () => {
  try {
    ...
    const response = await handleEveTitle(request({ text: 'Bitte plane den Launch' }), { fetch: fetchStub });
    const body = await response.json();
