...
async function callOpenRouterTitle(args: {
  ...
  { ok: true; title: string } | { ok: false; error: string; status: number }
> {
  ...
  try {
    const response = await args.fetchFn(OPENROUTER_CHAT_COMPLETIONS_URL, {
      ...
      body: JSON.stringify({
        model: Deno.env.get("EVE_TITLE_OPENROUTER_MODEL") ||
          EVE_TITLE_DEFAULT_MODEL,
    ...
    const content =
      (raw as { choices?: Array<{ message?: { content?: unknown } }> } | null)
...
export async function handleEveTitle(
  req: Request,
  deps: EveTitleHandlerDeps = {},
): Promise<Response> {
  ...
  return jsonResponse(
    req,
    { ok: true, title: title.title, checked_at: now },
    200,
  );
}
