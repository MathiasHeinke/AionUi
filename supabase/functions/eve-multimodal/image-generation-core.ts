...
import { type ImageModelRegistryEntry, resolveImageGenerationModel } from './image-model-registry.ts';
 ...
 */
const IMAGE_GENERATION_OPERATION = billableOperation('multimodal.image_generation')!;
...
function decodeBase64(value: unknown): Uint8Array | null {
  if (
    typeof value !== 'string' ||
    value.length < 4 ||
    value.length > Math.ceil((EVE_IMAGE_GENERATION_MAX_REFERENCE_BYTES * 4) / 3) + 8 ||
...
export function extractEveImageGenerationInput(body: unknown): EveImageGenerationInput | null {
  if (!isRecord(body)) return null;
  const prompt = normalizePrompt(body.prompt);
  if (!prompt || prompt.length > EVE_IMAGE_GENERATION_MAX_PROMPT_CHARS) {
    return null;
  }
  const aspectRatio = typeof body.aspect_ratio === 'string' ? body.aspect_ratio.trim() : '16:9';
  const resolution = typeof body.resolution === 'string' ? body.resolution.trim() : '1K';
  if (!ALLOWED_ASPECT_RATIOS.has(aspectRatio) || !ALLOWED_RESOLUTIONS.has(resolution)) return null;
  ...
  for (const reference of rawReferences) {
    ...
    const mimeType = typeof reference.mime_type === 'string' ? reference.mime_type.toLowerCase().trim() : '';
    const sha256 = typeof reference.sha256 === 'string' ? reference.sha256.toLowerCase().trim() : '';
...
function extractProviderImage(value: unknown): OpenRouterGeneratedImage | null {
  ...
  const mimeType = typeof image.media_type === 'string' ? image.media_type.toLowerCase().trim() : 'image/png';
  if (!ALLOWED_MIME_TYPES.has(mimeType)) return null;
  const bytes = decodeBase64(image.b64_json);
  if (!bytes || bytes.byteLength < 8 || bytes.byteLength > EVE_IMAGE_GENERATION_MAX_OUTPUT_BYTES) return null;
  const usage = isRecord(value.usage) ? value.usage : undefined;
  const rawCost = usage?.cost;
  const costUsd =
    typeof rawCost === 'number' && Number.isFinite(rawCost) && rawCost >= 0 && rawCost <= 10 ? rawCost : undefined;
  return {
    bytes,
...
export async function callOpenRouterImageGeneration(args: {
  ...
  { ok: true; image: OpenRouterGeneratedImage } | { ok: false; status: number; reason: string; message: string }
> {
  ...
  try {
    const response = await billedFetch({
      ...
      init: {
        ...
        headers: {
          ...
          'X-Title': Deno.env.get('OPENROUTER_X_TITLE') ?? 'Command EVE Visual Direction Gate',
        },
        body: JSON.stringify({
    ...
    if (Number.isFinite(contentLength) && contentLength > EVE_IMAGE_GENERATION_MAX_PROVIDER_RESPONSE_BYTES) {
      return {
        ok: false,
        status: 502,
    ...
    if (new TextEncoder().encode(responseText).byteLength > EVE_IMAGE_GENERATION_MAX_PROVIDER_RESPONSE_BYTES) {
      return {
        ok: false,
        status: 502,
        reason: 'provider-response-too-large',
    ...
    if (!response.ok) {
      return {
        ...
        reason: response.status === 429 ? 'provider-rate-limit' : 'provider-error',
        message: `OpenRouter image generation returned HTTP ${response.status}.`,
      };
    }
    ...
    if (!image) {
      return {
        ...
        message: 'OpenRouter image generation returned an invalid image artifact.',
      };
    }
    ...
    if (error instanceof UnbilledCallError) {
      return {
        ...
        message: `The image-generation call was refused before the provider: ${error.code}.`,
      };
    }
    ...
    return {
      ...
      message: aborted ? 'OpenRouter image generation timed out.' : 'OpenRouter image generation request failed.',
    };
  } finally {
...
export function imageGenerationPromptSha256(input: EveImageGenerationInput): string {
