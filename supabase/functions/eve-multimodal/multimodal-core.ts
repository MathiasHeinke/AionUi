...
function response(
  ...
  extra: Partial<
    Pick<
      ...
      'capability' | 'residency' | 'artifact' | 'tts' | 'document' | 'vision' | 'image_generation'
    >
  > = {},
...
const VISION_MIME_TYPES = Object.freeze(['image/jpeg', 'image/png', 'image/webp'] as const);
...
function cleanVisionFileName(value: unknown, sourceKind: EveMultimodalVisionSourceKind): string {
  if (typeof value !== 'string') return '';
  const base = value.trim().replace(/\\/g, '/').split('/').pop() || '';
  const extension = base.toLowerCase().match(/\.[a-z0-9]+$/)?.[0] ?? '';
  if (
    (sourceKind === 'presentation' && extension !== '.pptx') ||
    (sourceKind === 'image' && !['.jpg', '.jpeg', '.png', '.webp'].includes(extension))
  )
    return '';
  return stripAsciiControlCharacters(base).slice(0, 180);
}
...
function decodeVisionImageBase64(value: unknown): Uint8Array | null {
  ...
  try {
    const binary = atob(value);
    if (binary.length === 0 || binary.length > EVE_MULTIMODAL_VISION_MAX_IMAGE_BYTES) return null;
    return Uint8Array.from(binary, (character) => character.charCodeAt(0));
  } catch {
    return null;
  }
...
function hasVisionImageMagic(bytes: Uint8Array, mimeType: EveMultimodalVisionMimeType): boolean {
...
export function extractEveMultimodalVisionInput(body: unknown): EveMultimodalVisionInput | null {
  if (!isRecord(body)) return null;
  const sourceKind =
  ...
  const locale =
    body.locale === 'en-US' ? 'en-US' : body.locale === undefined || body.locale === 'de-DE' ? 'de-DE' : null;
  if (
    !fileName ||
    !isSha256(body.file_sha256) ||
  ...
  if (sourceKind === 'image' && (Number(slideCount) !== 1 || images.length !== 1)) return null;
...
export function decideEveMultimodalSkeletonRequest(args: DecideEveMultimodalArgs): EveMultimodalDecision {
  ...
  if (
    ...
    (provider === 'openrouter' &&
      args.body.capability !== 'document_ocr' &&
  ...
  if (privacyLane === 'local_only') {
    return response(
      ...
      'local-only-privacy',
      `${
        provider === 'openrouter' ? 'OpenRouter multimodal processing' : 'xAI multimodal'
  ...
  const pdfInput =
    provider === 'openrouter' && capability === 'document_ocr' ? extractEveMultimodalPdfInput(args.body) : null;
  ...
  const visionInput =
    provider === 'openrouter' && capability === 'vision' ? extractEveMultimodalVisionInput(args.body) : null;
  ...
  const imageGenerationInput =
    provider === 'openrouter' && capability === 'image_generation' ? extractEveImageGenerationInput(args.body) : null;
  if (provider === 'openrouter' && capability === 'image_generation' && !imageGenerationInput) {
    return response(
      400,
      'invalid-request',
      ...
      ...(visionInput
        ? {
            vision: {
              ...
              input_bytes: visionInput.images.reduce((sum, image) => sum + image.bytes.byteLength, 0),
            },
          }
        : {}),
