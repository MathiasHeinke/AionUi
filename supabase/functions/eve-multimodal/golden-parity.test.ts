...
function requestFor(testCase: GoldenCase, wire: string): Request {
  ...
  return new Request('https://example.supabase.co/functions/v1/eve-multimodal', {
    method: 'POST',
    headers,
    body: 'raw_body' in testCase ? testCase.raw_body : JSON.stringify(testCase.request),
  });
}
...
for (const testCase of golden.cases) {
  Deno.test(`preserves pre-split golden contract: ${testCase.id}`, async () => {
    ...
    try {
      ...
      assertEquals(JSON.stringify(responseBody).includes('Golden TTS text.'), false);
      assertEquals(JSON.stringify(responseBody).includes('golden-test-xai-key'), false);
    } finally {
