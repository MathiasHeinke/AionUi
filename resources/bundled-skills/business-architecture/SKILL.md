...
description: Design a business (or a single offer) to be SCALABLE and SELLABLE — structured around a system, not around the founder. Distills the Exit-Mindset (build as if you'd sell tomorrow, even if you never sell — that standard forces systems, SOPs, no key-person risk), the Sellability Audit (the once-a-year scorecard of what a buyer would actually pay for), the Soft-Shop Strategy (simulate a sale with real buyers to get the world's most honest free strategy consulting — the "magic question" turns a valuation into next year's business plan), Scalability / the Don-Shula touchdown filter (choose initiatives whose BEST case is a 10x, not a 5% lift), and the Three-Paths growth/exit-optionality framing (self-fund / outside equity / employee-ownership). Use when structuring or restructuring a business or an offer, packaging a service into a productised asset, or when the user asks "how do I make this scalable / sellable", "is my business worth anything", "how do I grow this", "what would a buyer pay". BILINGUAL (DE + EN operators). Feeds gtm-strategy (the architecture sets what's sellable) and plan-system (the two biggest audit gaps become versioned milestones). Source: AI Business Blueprint (FYN Labs, public) + Chiron-Framework (DE).
---
...
1. **Run the SELLABILITY AUDIT (what's sellable today).** Score 1–10 on the criteria a buyer actually checks:
   documented processes/SOPs · **no concentration risk** (no single customer >20%) · delivery skill on **more than
   one** person · recurring/predictable revenue · clean brand-IP, domains, contracts · clean financials (P&L,
   bookkeeping). The test for every structural decision: not "does this work?" but **"would this give an outside
   buyer confidence?"**. The **two lowest scores become next quarter's priorities** — hand them to `plan-system`.
...
- **Alois on his own business first.** The operator (Alois) runs it on **his** agency: where is his key-person risk, is any one client >20% of revenue, which of his offers is a touchdown vs a yard play, and what two things would a buyer pay more for — the two lowest audit scores feed straight into `plan-system` as versioned milestones, and the locked architecture feeds `gtm-strategy` (you can only go to market with what's actually sellable).
- **Then Alois on each client.** Once Alois has clients in EVE, he points the same audit + touchdown filter at a **client's** business or offer to package it as a productised, sellable asset — turning "I do custom work" into a transferable system. EVE keeps strict per-client isolation and never brands to or poaches the end-client (`eve-doctrine`).
- **Evidence rule.** A valuation or "buyer would pay X" claim is a **hypothesis until a real buyer says it** — EVE marks Soft-Shop numbers as simulated until grounded in an actual buyer conversation, and uses `decision-brief` to hand the operator one clear next move rather than a menu.
